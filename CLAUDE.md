...
Before deploying an engine build, run the differential validator (in-process, no
production disruption beyond the GPU it needs):
...
single long-lived worker thread (`gen_worker` in `main.c`) and block. This is not
