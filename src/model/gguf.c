...
static char *rd_str(FILE *f) {
    ...
    if (n > (1u << 28) || n > SIZE_MAX - 1) return NULL;  // sanity
    char *s = (char *)malloc((size_t)n + 1);
    if (!s) return NULL;
    if (!rd(f, s, (size_t)n)) { free(s); return NULL; }
    s[n] = '\0';
    return s;
}
...
static bool rd_kv(FILE *f, gguf_kv *kv) {
    ...
    if (kv->type == GGUF_ARRAY) {
        ...
        if (kv->arr_type == GGUF_STRING) {
            ...
            if (n > SIZE_MAX / sizeof(char *)) return false;
            kv->arr_str = (char **)calloc(
                (size_t)(n ? n : 1), sizeof(char *));
            if (!kv->arr_str) return false;
            ...
            if (n > SIZE_MAX / sizeof(int32_t)) return false;
            size_t bytes = (size_t)n * sizeof(int32_t);
            kv->arr_i32 = (int32_t *)malloc(bytes ? bytes : 1);
            if (!kv->arr_i32 || !rd(f, kv->arr_i32, bytes)) return false;
        } else {
            // skip other element types (not needed by ember)
            size_t sz = type_size(kv->arr_type);
            if (sz == 0 || n > (uint64_t)LONG_MAX / sz) return false;
...
gguf_file *gguf_open(const char *path) {
    ...
    if (file_size_l < 0 || fseek(f, 0, SEEK_SET) != 0) {
        fclose(f);
        return NULL;
    }
    ...
    // This also blocks multiplication wrap and overcommit-sized allocations on
    // a corrupt or attacker-supplied model file.
    const uint64_t max_entries = 1u << 20;
...
void gguf_free(gguf_file *g) {
    ...
    free(g->kv);
    for (uint64_t i = 0; g->tensors && i < g->n_tensors; i++)
