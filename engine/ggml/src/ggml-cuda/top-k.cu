...
// NOTE: GGML_CUDA_USE_HIPCUB is deliberately NOT accepted here. common.cuh
// defines it unconditionally on HIP, so including it made this branch shadow the
// #else below — which is the branch actually written for HIP (it dispatches
// ncols > 1024 to hipCUB itself and keeps the dedicated partial-bitonic top-k,
// plus a wave-shuffle argmax for k == 1, for the common decode/verify case).
// The result was that every DS4 indexer top-k fully sorted the row and then
// discarded all but k indices. HIP now falls through to #else as intended.
