...
#endif
#define LUCEBOX_RDNA_TILE_HOST(cc) (LUCEBOX_RDNA_MMQ_TILE_OVERRIDE && GGML_CUDA_CC_IS_GFX1151(cc))
#if LUCEBOX_RDNA_MMQ_TILE_OVERRIDE && defined(RDNA3)
...
static constexpr __host__ __device__ tile_x_sizes mmq_get_dp4a_tile_x_sizes(ggml_type type, int mmq_y) {
    switch (type) {
        ...
        case GGML_TYPE_Q2_0_ROCMFP2: return MMQ_DP4A_TXS_Q4_1;
...
static constexpr __host__ __device__ int mmq_get_mma_tile_x_k(ggml_type type, int cc) {
    switch (type) {
...
#endif
        case GGML_TYPE_Q2_0_ROCMFP2: return MMQ_MMA_TILE_X_K_Q8_1;
...
// affine fp2 MMQ (q4_1-style: scale, -offset; 2-bit codes)
template <int mmq_y, bool need_check> static __device__ __forceinline__ void load_tiles_rocmfp2_affine(
    const char * __restrict__ x, int * __restrict__ x_tile, const int kbx0, const int i_max, const int stride) {
    constexpr int nwarps = mmq_get_nwarps_device();
    constexpr int warp_size = ggml_cuda_get_physical_warp_size();
...
#pragma unroll
    for (int i0 = 0; i0 < mmq_y; i0 += nrows*nwarps) {
        ...
        const int q_lo = rocmfpx_pack4_fp2_vec_cuda(bxi->qs, 4*kqsx);
        const int q_hi = rocmfpx_pack4_fp2_vec_cuda(bxi->qs, 4*kqsx + 16);

#if defined(AMD_WMMA_AVAILABLE)
        x_qs[i*MMQ_MMA_TILE_X_K_Q8_1 + kbx*(2*QI4_1) + kqsx + 0]     = q_lo;
        x_qs[i*MMQ_MMA_TILE_X_K_Q8_1 + kbx*(2*QI4_1) + kqsx + QI4_1] = q_hi;
#else
        x_qs[i*(MMQ_TILE_NE_K + 1) + txi] = q_lo; (void) q_hi;
#endif // defined(AMD_WMMA_AVAILABLE)
    }
...
#pragma unroll
    for (int i0 = 0; i0 < mmq_y; i0 += nwarps * rows_per_warp) {
        ...
        const half2 dm = __floats2half2_rn(rocmfpx_ue4m3_to_fp32_finite(bxi->e[0]), -rocmfpx_ue4m3_to_fp32_finite(bxi->e[1]));
...
#pragma unroll
    ...
    const int kbxd = threadIdx.x % blocks_per_row;
#pragma unroll
    for (int i0 = 0; i0 < mmq_y; i0 += nwarps*rows_per_warp) {
        int i = i0 + threadIdx.y*rows_per_warp + threadIdx.x/blocks_per_row;
        if (need_check) i = min(i, i_max);
        const block_rocmi4 * bxi = (const block_rocmi4 *) x + kbx0 + i*stride + kbxd;
...
static __device__ __forceinline__ void vec_dot_q8_0_16_q8_1_mma(
    const int * __restrict__ x, const int * __restrict__ y, float * __restrict__ sum, const int k00) {
#if   defined(AMD_WMMA_AVAILABLE) //wmma instructions can handle 16x4 tiles, does not require loading 64x2 tiles
    constexpr data_layout input_layout = get_input_data_layout();
...
struct mmq_args {
    const char * x; const char * weight_name; ggml_type type_x; const int * y; const int32_t * ids_dst; const int32_t * expert_bounds; float * dst;
...
static __device__ __forceinline__ void vec_dot_rocmi4_w4a4_wmma(
    ...
    constexpr data_layout layout = get_input_data_layout();
    typedef tile<16, 4, int, layout> tile_A;
    typedef tile<16, 4, int, layout> tile_B;
    typedef tile<16, 16, int, DATA_LAYOUT_J_MAJOR> tile_C;
    constexpr int granularity = mmq_get_granularity_device(mmq_x);
    constexpr int rows_per_warp = granularity;
    constexpr int ntx = rows_per_warp/tile_C::I;
...
#pragma unroll
                for (int l = 0; l < tile_C::ne; ++l) {
                    const int i = i0 + n*tile_A::I + tile_C::get_i(l);
                    const float dA = x_df[i*MMQ_MMA_TILE_X_K_ROCMI4 + k0/QI8_0];
                    sum[(j0/tile_C::J + n)*tile_C::ne + l] += (C.x[l]*16)*dA*dB;
                }
            }
        }
    }
