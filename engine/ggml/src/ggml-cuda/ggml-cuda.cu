...
static cudaError_t ggml_cuda_device_malloc(void ** ptr, size_t size, int device, bool * out_managed = nullptr) {
    ...
    if (ggml_cuda_device_use_uma(device, size)) {
        ...
        if (managed && err == cudaSuccess && getenv("DFLASH_HIP_NO_MLOCK") == nullptr) {
            if (mlock(*ptr, size) != 0) {
                GGML_LOG_WARN("dflash: mlock(%.1f GiB managed) failed: %s (raise memlock ulimit)\n",
                              (double) size / 1073741824.0, strerror(errno));
            } else {
                GGML_LOG_INFO("dflash: mlock pinned %.1f GiB managed buffer (unswappable)\n",
...
struct ggml_cuda_pool_leg : public ggml_cuda_pool {
    ...
    void free(void * ptr, size_t size) override {
        // Bound the pool (GGML_HIP_NO_VMM path): return excess freed buffers to the
        // driver instead of hoarding them forever, which otherwise grows GTT/RAM to
        // the peak of all variable-size large-context ops and never shrinks.
        // Tunable via GGML_CUDA_POOL_MAX_MB (0 = unbounded/legacy behaviour).
        static const size_t pool_cap = []() {
            const char * e = getenv("GGML_CUDA_POOL_MAX_MB");
            return (size_t)(e ? atoll(e) : 6144) * 1024ull * 1024ull;
        }();
        if (pool_cap > 0 && pool_size > pool_cap) {
            ggml_cuda_set_device(device);
...
static bool ggml_backend_cuda_device_supports_op(ggml_backend_dev_t dev, const ggml_tensor * op) {
    ...
    switch (op->op) {
        ...
        case GGML_OP_RMS_NORM:
        case GGML_OP_L2_NORM:
            // ROCmFPX 5ed0d9e: norm kernels index the innermost dimension
            // directly. A permuted row would assert inside norm.cu instead of
