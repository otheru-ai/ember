...
// Ported from lucebox 71c2ecf (fix(ds4): harden heterogeneous GPU correctness).
...
#pragma unroll
    ...
    // lucebox 71c2ecf: value_bounds is written by a subset of threads and read
    // by all of them below; without this barrier the read races the write.
    __syncthreads();
...
// ─────────────── q=1 decode attention via WMMA (gfx11 / RDNA3+) ───────────────
// MLA decode is a matrix-matrix problem even at q=1: 64 query heads share one
// latent KV head, so operational intensity is H_qo/H_kv = 64. The dense D=512
// kernels above reuse K/V across the *query* dimension, which is empty here, so
// they re-read the whole span once per head-block. This one keeps all 64 heads
// resident and reads the span once.
//
// Fragment layouts are the verified ones (ISA 7.9 VGPR-view figures, confirmed
// by the AMD Matrix Instruction Calculator and CK's wmma_gemm.hpp):
//   A: lane L holds row L%16, all 16 K values
//   B: lane L holds column L%16
//   D: c_frag[i] = D[2i + lane/16][lane%16]
//   A/B replicate across lanes 0-15 / 16-31, which loading with lane%16 gives.
// __gfx11__ is NOT a compiler predefine -- composable_kernel defines it itself
// from the per-target macros. Guarding on it directly silently compiles the
// WMMA intrinsics out of the device pass, leaving a kernel that runs fast and
// returns nonsense. Use the actual gfx1151 compiler predefine.
#if defined(__gfx1151__)
#define DS4_HAVE_WMMA 1
#endif

#define DS4_DECW_D       512
#define DS4_DECW_THREADS 512
#define DS4_DECW_WAVES   (DS4_DECW_THREADS / 32)
#define DS4_DECW_TR      16
#define DS4_DECW_NEG     (-3.402823466e38f)

typedef _Float16 ds4_half16 __attribute__((ext_vector_type(16)));
typedef float    ds4_float8 __attribute__((ext_vector_type(8)));

template <typename KV, int NHEAD>
__global__ __launch_bounds__(DS4_DECW_THREADS) void ds4_decode_attn_wmma_partial(
        float * __restrict__ part_acc, float * __restrict__ part_m,
        float * __restrict__ part_l,
        const half * __restrict__ qs,   // [NHEAD][D], pre-converted
        const KV * __restrict__ k, const half * __restrict__ mask,
        int n_kv, int n_split, float scale) {
    constexpr int D  = DS4_DECW_D;
    constexpr int MT = NHEAD / 16;
    constexpr int NT = D / 16;
    constexpr int NW = DS4_DECW_WAVES;
    constexpr int TR = DS4_DECW_TR;
    constexpr int KCH = NW / MT;              // k-chunks per M-tile
    constexpr int APW = NT / NW;              // accumulator tiles per wave

    const int s = blockIdx.x, tid = threadIdx.x;
    const int wave = tid / 32, lane = tid % 32, l16 = lane % 16, hh = lane / 16;

    const int rps = (n_kv + n_split - 1) / n_split;
    const int r_begin = s * rps, r_end = min(n_kv, r_begin + rps);

    extern __shared__ char ds4_smem[];
    float * sc  = (float *) ds4_smem;                       // KCH * NHEAD * TR
    half  * pt  = (half *) (sc + KCH * NHEAD * TR);         // NHEAD * TR
    float * mr  = (float *) (pt + NHEAD * TR);
    float * lr  = mr + NHEAD;
    float * al  = lr + NHEAD;
    for (int i = tid; i < NHEAD; i += DS4_DECW_THREADS) { mr[i] = DS4_DECW_NEG; lr[i] = 0.0f; }

    ds4_float8 acc[MT][APW];
#pragma unroll
    for (int m = 0; m < MT; ++m)
#pragma unroll
        for (int n = 0; n < APW; ++n) acc[m][n] = (ds4_float8)(0.0f);
    __syncthreads();

    for (int r0 = r_begin; r0 < r_end; r0 += TR) {
        const int rows = min(TR, r_end - r0);
        {
            const int qm = wave % MT, kc = wave / MT;
            constexpr int KS = (D / 16) / KCH;
            ds4_float8 sa = (ds4_float8)(0.0f);
#pragma unroll 4
            for (int kk = qm * 0 + kc * KS; kk < (kc + 1) * KS; ++kk) {
                const ds4_half16 a = *(const ds4_half16 *)
                    (qs + (size_t) (qm * 16 + l16) * D + kk * 16);
                ds4_half16 b;
                const KV * kr = k + (size_t) min(r0 + l16, n_kv - 1) * D + kk * 16;
#pragma unroll
                for (int j = 0; j < 16; ++j) b[j] = (_Float16) (float) kr[j];
#if defined(DS4_HAVE_WMMA)
                sa = __builtin_amdgcn_wmma_f32_16x16x16_f16_w32(a, b, sa);
#else
                (void) a; (void) b;   // host pass: symbol only
#endif
            }
#pragma unroll
            for (int i = 0; i < 8; ++i)
                sc[kc * NHEAD * TR + (qm * 16 + 2 * i + hh) * TR + l16] = sa[i];
        }
        __syncthreads();

        {
            constexpr int LPH = DS4_DECW_THREADS / NHEAD;
            constexpr int RPL = TR / LPH;
            const int sh = tid / LPH, ss = tid % LPH;
            float v[RPL], tmax = DS4_DECW_NEG;
#pragma unroll
            for (int t = 0; t < RPL; ++t) {
                const int r = ss * RPL + t;
                float raw = 0.0f;
#pragma unroll
                for (int c = 0; c < KCH; ++c) raw += sc[c * NHEAD * TR + sh * TR + r];
                const float mv = (r < rows) ? __half2float(mask[r0 + r]) : -1.0e30f;
                v[t] = mv > -1.0e20f ? raw * scale + mv : DS4_DECW_NEG;
                tmax = fmaxf(tmax, v[t]);
            }
#pragma unroll
            for (int o = 1; o < LPH; o <<= 1) tmax = fmaxf(tmax, __shfl_xor(tmax, o, LPH));
            const float mo = mr[sh], mn = fmaxf(mo, tmax);
            const float a = mo == DS4_DECW_NEG ? 0.0f : __expf(mo - mn);
            float sum = 0.0f;
#pragma unroll
            for (int t = 0; t < RPL; ++t) {
                const float p = v[t] == DS4_DECW_NEG ? 0.0f : __expf(v[t] - mn);
                pt[sh * TR + ss * RPL + t] = __float2half(p);
                sum += p;
            }
#pragma unroll
            for (int o = 1; o < LPH; o <<= 1) sum += __shfl_xor(sum, o, LPH);
            if (ss == 0) { mr[sh] = mn; lr[sh] = lr[sh] * a + sum; al[sh] = a; }
        }
        __syncthreads();
...
#pragma unroll
        for (int m = 0; m < MT; ++m) {
            float av[8];
#pragma unroll
            for (int i = 0; i < 8; ++i) av[i] = al[m * 16 + 2 * i + hh];
            const ds4_half16 a = *(const ds4_half16 *) (pt + (size_t) (m * 16 + l16) * TR);
#pragma unroll
            for (int n = 0; n < APW; ++n) {
                const int nt = wave * APW + n;
                ds4_half16 b;
#pragma unroll
                for (int j = 0; j < 16; ++j)
                    b[j] = (_Float16) (float) k[(size_t) min(r0 + j, n_kv - 1) * D + nt * 16 + l16];
#pragma unroll
                for (int i = 0; i < 8; ++i) acc[m][n][i] *= av[i];
#if defined(DS4_HAVE_WMMA)
                acc[m][n] = __builtin_amdgcn_wmma_f32_16x16x16_f16_w32(a, b, acc[m][n]);
#else
                (void) a; (void) b;
#endif
            }
...
#pragma unroll
            for (int i = 0; i < 8; ++i)
                part_acc[((size_t) s * NHEAD + m * 16 + 2 * i + hh) * D + nt * 16 + l16] =
                    acc[m][n][i];
        }
    if (tid < NHEAD) {
        part_m[(size_t) s * NHEAD + tid] = mr[tid];
        part_l[(size_t) s * NHEAD + tid] = lr[tid];
    }
}

template <int NHEAD>
__global__ void ds4_decode_attn_q_to_f16(
        half * __restrict__ out, const float * __restrict__ q, size_t stride_head) {
    const int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= NHEAD * DS4_DECW_D) return;
    out[i] = __float2half(q[(size_t) (i / DS4_DECW_D) * stride_head + (i % DS4_DECW_D)]);
}

template <int NHEAD>
__global__ __launch_bounds__(256) void ds4_decode_attn_wmma_combine(
        float * __restrict__ dst, const float * __restrict__ pa,
        const float * __restrict__ pm, const float * __restrict__ pl,
        const float * __restrict__ sinks, int n_split) {
    const int h = blockIdx.x, tid = threadIdx.x;
    __shared__ float mm, ll;
    if (tid == 0) {
        float m = sinks ? sinks[h] : DS4_DECW_NEG;
        for (int s = 0; s < n_split; ++s) m = fmaxf(m, pm[(size_t) s * NHEAD + h]);
        float l = (sinks && m > DS4_DECW_NEG) ? __expf(sinks[h] - m) : 0.0f;
        for (int s = 0; s < n_split; ++s) {
            const float ms = pm[(size_t) s * NHEAD + h];
            if (ms == DS4_DECW_NEG) continue;
    ...
    for (int d = tid; d < DS4_DECW_D; d += 256) {
        float v = 0.0f;
        for (int s = 0; s < n_split; ++s) {
            const float ms = pm[(size_t) s * NHEAD + h];
            if (ms == DS4_DECW_NEG) continue;
            v += pa[((size_t) s * NHEAD + h) * DS4_DECW_D + d] * __expf(ms - mm);
        }
        dst[(size_t) h * DS4_DECW_D + d] = v * inv;
    }
}
...
static bool ggml_cuda_ds4_flash_attn_d512_f32(
    ...
    // q=1 decode: the dense kernels below reuse K/V across the query dimension,
    // which is empty here, so they re-read the span once per head-block. This
    // path keeps all heads resident and reads it once.
    // NB: gate on the runtime arch, not __gfx11__ -- that macro is defined only
    // in the device compilation pass, so using it here silently removes the
    // dispatch from the host pass and the kernel never runs.
    // DFLASH_DS4_DECODE_WMMA=0 keeps the F16-K graph path but routes q=1 back
    // to the dense kernels, to separate a graph fault from a kernel fault.
    static const bool dec_wmma_enabled = []() {
        const char * e = getenv("DFLASH_DS4_DECODE_WMMA");
        return !(e && e[0] == '0');
    }();
    const int dec_cc = ggml_cuda_info().devices[ggml_cuda_get_device()].cc;
    float dec_params[3] = {};
    memcpy(dec_params, dst->op_params, sizeof(dec_params));
    const float dec_scale = dec_params[0];
    if (dec_wmma_enabled &&
        n_tokens == 1 && n_heads == 64 && !sparse && !indexed_mask && mask &&
        mask->type == GGML_TYPE_F16 && rope_flags == 0 && n_kv >= 16 &&
        GGML_CUDA_CC_IS_GFX1151(dec_cc)) {
        constexpr int NH = 64;
        ...
        // split-dependent divergence points there rather than at the maths.
        static const int split_override = []() {
            const char * e = getenv("DFLASH_DS4_DECODE_SPLIT");
            return e ? atoi(e) : 0;
        }();
        int split = 1;
        while (split < 32 && n_kv / (split * 2) >= 256) split *= 2;
        if (split_override > 0) split = split_override;
        const size_t shmem_dec =
            (size_t) (DS4_DECW_WAVES / (NH / 16)) * NH * DS4_DECW_TR * sizeof(float) +
            (size_t) NH * DS4_DECW_TR * sizeof(half) + 3 * NH * sizeof(float);
        if (shmem_dec <= 64 * 1024) {
            // Fixed sizes regardless of split: n_kv differs per layer, so a
            // split-sized request would make the pool churn between block sizes
            // on every one of the model's 43 attention nodes, every token.
            constexpr int MAXSPLIT = 32;
            ggml_cuda_pool_alloc<half>  q16(ctx.pool(), (size_t) NH * 512);
            ggml_cuda_pool_alloc<float> pa(ctx.pool(), (size_t) MAXSPLIT * NH * 512);
            ggml_cuda_pool_alloc<float> pm(ctx.pool(), (size_t) MAXSPLIT * NH);
            ggml_cuda_pool_alloc<float> pl(ctx.pool(), (size_t) MAXSPLIT * NH);
            ds4_decode_attn_q_to_f16<NH><<<(NH * 512 + 255) / 256, 256, 0, stream>>>(
                q16.get(), (const float *) Q->data, q_stride_head);
            if (kv_f16) {
                ds4_decode_attn_wmma_partial<half, NH>
                    <<<split, DS4_DECW_THREADS, shmem_dec, stream>>>(
                        pa.get(), pm.get(), pl.get(), q16.get(),
                        (const half *) K->data, (const half *) mask->data,
                        n_kv, split, dec_scale);
            } else {
                ds4_decode_attn_wmma_partial<float, NH>
                    <<<split, DS4_DECW_THREADS, shmem_dec, stream>>>(
                        pa.get(), pm.get(), pl.get(), q16.get(),
                        (const float *) K->data, (const half *) mask->data,
                        n_kv, split, dec_scale);
            }
            ds4_decode_attn_wmma_combine<NH><<<NH, 256, 0, stream>>>(
                (float *) dst->data, pa.get(), pm.get(), pl.get(),
                sinks ? (const float *) sinks->data : nullptr, split);
            return true;
        }
    }
