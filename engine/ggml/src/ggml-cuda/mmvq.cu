...
static constexpr __device__ int get_mmvq_mmid_max_batch_for_device() {
    // RDNA3 is defined on gfx1151, so this was always the RDNA3 table.
    return get_mmvq_mmid_max_batch_rdna3(type);
}
...
static __global__ void mul_mat_vec_q(
    ...
    constexpr int nwarps = 1; // gfx1151 parameter table
...
static std::pair<dim3, dim3> calc_launch_params(
        const int64_t nrows_x, const int nchannels_dst, const int nsamples_or_ntokens,
        const int warp_size) {
    // gfx1151's established layout uses one wave and one row per block.
    return {dim3(nrows_x, nchannels_dst, nsamples_or_ntokens), dim3(warp_size, 1, 1)};
}
...
static void mul_mat_vec_q_switch_fusion(
    ...
    const bool has_fusion = fusion.gate != nullptr || fusion.x_bias != nullptr || fusion.gate_bias != nullptr;
    if (has_fusion) {
        mul_mat_vec_q<type, c_ncols_dst, true, fixed_ncols_x,
    ...
    mul_mat_vec_q<type, c_ncols_dst, false, fixed_ncols_x,
                  unroll_k_loop_2, reuse_rocmfp4_weights>
        <<<block_nums, block_dims, nbytes_shared, stream>>>
        (vx, vy, ids, fusion, dst, ncols_x, nchannels_y, stride_row_x, stride_col_y, stride_col_dst,
        channel_ratio, stride_channel_x, stride_channel_y, stride_channel_dst,
...
static void mul_mat_vec_rocmfpx_fixed_k_launch(
    ...
    mul_mat_vec_q_switch_fusion<type, 1, fixed_ncols_x>(
...
static void mul_mat_vec_rocmfp4_unroll2_launch(
    ...
    const dim3 block_nums(nrows_x, nchannels_dst, nsamples_dst);
    const dim3 block_dims(warp_size, 1, 1);
    mul_mat_vec_q_switch_fusion<GGML_TYPE_Q4_0_ROCMFP4_FAST, 1, 0, true>(
        vx, vy, ids, fusion, dst, ncols_x, nchannels_y, stride_row_x,
        stride_col_y, stride_col_dst, channel_ratio, stride_channel_x,
        stride_channel_y, stride_channel_dst, sample_ratio, stride_sample_x,
        stride_sample_y, stride_sample_dst, block_nums, block_dims, 0,
...
static void mul_mat_vec_rocmfp4_4col_reuse_launch(
    ...
    const auto dims = calc_launch_params(nrows_x, nchannels_dst, nsamples_dst, warp_size);
    mul_mat_vec_q_switch_fusion<
        GGML_TYPE_Q4_0_ROCMFP4_FAST, ncols_dst, 0, false, true>(
...
static void mul_mat_vec_q_switch_ncols_dst(
    ...
    if (use_tokenwise_mmid && has_ids && ncols_dst > 1) {
        ...
        std::pair<dim3, dim3> dims = calc_launch_params(nrows_x, nchannels_dst, ncols_dst, warp_size);
    ...
    switch (ncols_dst) {
        case 1: {
            ...
            std::pair<dim3, dim3> dims = calc_launch_params(nrows_x, nchannels_dst,
                                                                    nsamples_dst, warp_size);
            mul_mat_vec_q_switch_fusion<type, c_ncols_dst>(
        ...
        case 2: {
            ...
            std::pair<dim3, dim3> dims = calc_launch_params(nrows_x, nchannels_dst, nsamples_dst, warp_size);
