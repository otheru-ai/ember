...
// ── Affine fp2: value = code*scale - offset, codes {0,1,2,3} ────────────────
// This MUST match the GPU dequantizer (ggml-cuda/dequantize.cuh,
// dequantize_rocmfpx_fp2): e[0] is the scale, e[1] is the offset, and the whole
// 32-weight block shares both.
//
// An earlier revision read e[0]/e[1] as two per-half scales against a
// {0,1,2,3} codebook with no offset. That disagreed with the GPU about the
// meaning of the same bytes, and being offset-free it could only ever emit
// NON-NEGATIVE values - so any CPU-side read of an affine-quantized tensor
// (ggml to_float) produced garbage.
//
// Two bits is four levels, so none may be wasted: fit them to the block's real
// range instead of assuming it is centred on zero.
//   code 0 -> -offset          == min  =>  offset = -min
//   code 3 -> 3*scale - offset == max  =>  scale  = (max - min)/3
// e[] is UNSIGNED UE4M3, so offset >= 0; a block whose minimum is positive gets
// offset 0 and spans [0, 3*scale]. Same idea as ggml's Q2_K scale-and-min, in
// the ROCMFPx 10-byte block.
static inline uint8_t rocmfpx_affine_fp2_code(float x, float scale, float offset) {
    if (!isfinite(x) || !(scale > 0.0f)) {
        return 0;
    }
    int code = (int) lrintf((x + offset) / scale);
    if (code < 0) code = 0;
    if (code > 3) code = 3;
    return (uint8_t) code;
}

// Pick (scale, offset) for one block. Shrinking the fitted range trades
// clipping of the extremes against a finer step for the bulk; the (optionally
// imatrix-weighted) squared error decides which trade wins for this block.
static void rocmfpx_choose_affine_fp2(const float * x, int n, const float * weights,
                                      uint8_t * e_scale, uint8_t * e_offset) {
    static const float shrink[] = { 1.0f, 0.95f, 0.90f, 0.85f, 0.80f, 0.75f, 0.70f };

    float mn = 0.0f, mx = 0.0f;
    bool have = false;
    for (int i = 0; i < n; ++i) {
        if (!isfinite(x[i])) continue;
        if (!have) { mn = mx = x[i]; have = true; continue; }
        if (x[i] < mn) mn = x[i];
        if (x[i] > mx) mx = x[i];
    }

    if (!have || !(mx > mn)) {
        // Constant (or all non-finite) block: no spread to encode. scale 0 with
        // offset = -value reproduces it exactly for every code.
        *e_scale  = 0;
        *e_offset = rocmfpx_nearest_scale_ue4m3(have && mn < 0.0f ? -mn : 0.0f);
        return;
    }

    uint8_t best_s = 0, best_o = 0;
    float best_err = INFINITY;

    for (size_t k = 0; k < sizeof(shrink)/sizeof(shrink[0]); ++k) {
        const float mid       = 0.5f*(mn + mx);
        const float halfrange = 0.5f*(mx - mn)*shrink[k];
        const float lo = mid - halfrange;
        const float hi = mid + halfrange;

        const uint8_t es = rocmfpx_nearest_scale_ue4m3((hi - lo) / 3.0f);
        const uint8_t eo = rocmfpx_nearest_scale_ue4m3(lo < 0.0f ? -lo : 0.0f);
        const float scale  = rocmfpx_ue4m3_to_fp32(es);
        const float offset = rocmfpx_ue4m3_to_fp32(eo);

        float err = 0.0f;
        for (int i = 0; i < n; ++i) {
            if (!isfinite(x[i])) continue;
            const uint8_t code = rocmfpx_affine_fp2_code(x[i], scale, offset);
            const float diff = ((float) code * scale - offset) - x[i];
...
static void rocmfpx_quantize_row_fp2_weighted(
    ...
    for (int64_t ib = 0; ib < nb; ++ib) {
        ...
        // ggml's imatrix convention: w_i = imatrix_i * sqrt(sigma2 + x_i^2).
        float weights[QK_ROCMFP2];
        if (qw) {
            for (int i = 0; i < QK_ROCMFP2; ++i) {
                weights[i] = qw[i] * sqrtf(sigma2 + xb[i]*xb[i]);
            }
        }

        rocmfpx_choose_affine_fp2(xb, QK_ROCMFP2, qw ? weights : NULL, &yb->e[0], &yb->e[1]);
...
void rocmfpx_dequantize_row_fp2(const block_rocmfp2 * GGML_RESTRICT x, float * GGML_RESTRICT y, int64_t k) {
    ...
    for (int64_t ib = 0; ib < nb; ++ib) {
        ...
        // affine fp2: value = code*scale - offset ; e[0]=scale, e[1]=offset.
        // Must stay identical to dequantize_rocmfpx_fp2 in the CUDA/HIP path.
