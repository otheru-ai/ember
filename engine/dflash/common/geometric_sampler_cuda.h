...
inline bool gpu_sampler_supports(const SamplerCfg & cfg) {
    ...
    if (cfg.min_p > 0.0f) return false;                       // min_p: not implemented on GPU
    // DRY: not implemented on GPU. Without this the kernel would silently drop
    // the penalty -- the same "fast path ignores it" failure that
    // needs_logit_processing() guards against for greedy/DSpark. Dormant in
    // ember (HIP leaves DFLASH27B_HAVE_GPU_SAMPLER undefined) but the two
    // predicates have no mechanical link, so state it here too.
    if (cfg.dry_multiplier > 0.0f) return false;
    return true;
}
