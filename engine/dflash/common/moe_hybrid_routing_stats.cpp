...
bool MoeHybridRoutingStats::init(int n_layer_, int n_expert_, int n_expert_used_) {
    if (n_layer_ <= 0 || n_expert_ <= 0 || n_expert_used_ <= 0 ||
        n_expert_used_ > n_expert_ ||
        static_cast<size_t>(n_layer_) >
            std::numeric_limits<size_t>::max() /
            static_cast<size_t>(n_expert_)) {
        return false;
    }
    n_layer = n_layer_;
    n_expert = n_expert_;
...
bool MoeHybridRoutingStats::observe(int layer_idx, const int32_t * expert_ids, int n_ids) {
    ...
    std::vector<uint64_t> additions(static_cast<size_t>(n_expert), 0);
    for (int i = 0; i < n_ids; ++i) {
        const int expert_idx = expert_ids[i];
        if (expert_idx < 0 || expert_idx >= n_expert) {
            return false;
        }
        uint64_t & add = additions[static_cast<size_t>(expert_idx)];
        if (add == std::numeric_limits<uint64_t>::max()) return false;
        ++add;
    }
    if (static_cast<uint64_t>(n_ids) >
        std::numeric_limits<uint64_t>::max() -
            layer_totals[static_cast<size_t>(layer_idx)]) {
        return false;
    }
    for (int expert_idx = 0; expert_idx < n_expert; ++expert_idx) {
        const uint64_t add = additions[static_cast<size_t>(expert_idx)];
    ...
    for (int expert_idx = 0; expert_idx < n_expert; ++expert_idx) {
        counts[index_of(layer_idx, expert_idx)] +=
            additions[static_cast<size_t>(expert_idx)];
    }
    layer_totals[static_cast<size_t>(layer_idx)] +=
        static_cast<uint64_t>(n_ids);
    return true;
}
...
bool MoeHybridRoutingStats::observe_selected_tensor(ggml_backend_t backend,
    ...
    if (selected->ne[0] >
        std::numeric_limits<int64_t>::max() / selected->ne[1]) {
        if (err) *err = "selected tensor element count overflows";
        return false;
    }
    const int64_t n_ids = selected->ne[0] * selected->ne[1];
    if (n_ids > std::numeric_limits<int>::max() ||
        static_cast<uint64_t>(n_ids) >
            std::numeric_limits<size_t>::max() / sizeof(int32_t)) {
        if (err) *err = "selected tensor is too large";
        return false;
    }
...
bool MoeHybridRoutingStats::load_csv(const std::string & path,
    ...
    while (std::getline(f, line)) {
        if (line.empty() || line[0] == '#') {
            if (line.find("n_layer=") != std::string::npos) {
                if (std::sscanf(
                        line.c_str(),
                        "# hotness table: n_layer=%d n_expert=%d n_expert_used=%d",
                        &file_n_layer, &file_n_expert,
                        &file_n_expert_used) != 3 ||
                    file_n_layer <= 0 || file_n_expert <= 0 ||
                    file_n_expert_used <= 0 ||
                    file_n_expert_used > file_n_expert) {
                    if (err) *err = "invalid routing stats header";
                    return false;
                }
            }
            continue;
        ...
        while (*p) {
            ...
            if (*p == ',') {
                ++p;
                if (!*p) {
                    if (err) *err = "trailing comma in routing stats row";
                    return false;
                }
            } else if (*p) {
                if (err) *err = "malformed separator in routing stats row";
                return false;
            }
        }
        ...
        if (file_n_expert == 0) {
            if (row.size() >
                ...
                if (err) *err = "routing stats row is too wide";
                return false;
            }
            file_n_expert = static_cast<int>(row.size());
    ...
    const size_t detected_layers_sz =
        all_counts.size() / static_cast<size_t>(file_n_expert);
    if (detected_layers_sz >
        static_cast<size_t>(std::numeric_limits<int>::max())) {
        if (err) *err = "routing stats has too many layers";
        return false;
    }
    const int detected_layers = static_cast<int>(detected_layers_sz);
    if (file_n_layer == 0) file_n_layer = detected_layers;
    if (file_n_expert_used == 0) file_n_expert_used = 8;  // default

    if (file_n_layer <= 0 || file_n_expert <= 0 ||
        static_cast<size_t>(file_n_layer) >
            std::numeric_limits<size_t>::max() /
                static_cast<size_t>(file_n_expert) ||
        all_counts.size() !=
            static_cast<size_t>(file_n_layer) *
                static_cast<size_t>(file_n_expert)) {
    ...
    for (int il = 0; il < file_n_layer; ++il) {
        ...
        for (int ie = 0; ie < file_n_expert; ++ie) {
            ...
            if (count > std::numeric_limits<uint64_t>::max() - total) {
                if (err) *err = "routing stats layer total overflows";
                return false;
            }
            total += count;
        }
