...
void DaemonIO::emit(int32_t v) const {
    ...
    size_t remaining = sizeof(v);
#ifndef _WIN32
    while (remaining > 0) {
        const ssize_t n = ::write(stream_fd, src, remaining);
        if (n > 0) {
            src += n;
...
#else
    while (remaining > 0) {
        const int n = _write(stream_fd, src, static_cast<unsigned>(remaining));
        if (n <= 0) {
            cancelled = true;
            return;
        }
...
static bool command_is(const std::string & line, const char * command) {
    const size_t n = std::strlen(command);
    return line.size() >= n &&
           std::memcmp(line.data(), command, n) == 0 &&
           (line.size() == n || line[n] == ' ');
}
...
static std::vector<int32_t> read_uncounted_i32(const std::string & path) {
    ...
    if (end < 0) return {};
    const auto sz = static_cast<uintmax_t>(end);
    if (sz == 0 || sz % sizeof(int32_t) != 0 ||
        sz / sizeof(int32_t) >
    ...
    f.seekg(0);
    if (!f) return {};
    std::vector<int32_t> ids(static_cast<size_t>(sz / sizeof(int32_t)));
    if (!ids.empty()) {
...
static std::vector<int32_t> read_counted_i32(const std::string & path) {
    ...
    if (!f) return {};
    const std::streampos end = f.tellg();
    if (end < static_cast<std::streamoff>(sizeof(uint32_t))) return {};
    const uintmax_t file_size = static_cast<uintmax_t>(end);
    f.seekg(0);
    if (!f) return {};
    uint32_t n = 0;
    f.read(reinterpret_cast<char *>(&n), sizeof(n));
    if (!f) return {};
    const uintmax_t payload = static_cast<uintmax_t>(n) * sizeof(int32_t);
    if (payload != file_size - sizeof(uint32_t)) return {};
...
static bool write_counted_i32(const std::string & path,
    ...
    if (ids.size() > std::numeric_limits<uint32_t>::max()) return false;
...
int run_daemon(ModelBackend & backend, const DaemonLoopArgs & args) {
    ...
    while (std::getline(std::cin, line)) {
        ...
        if (starts_with(line, "SNAPSHOT ")) {
            int slot = -1;
            if (!parse_int_arg(line.c_str() + 9, slot) ||
                slot < 0 || slot >= ModelBackend::kMaxSlots ||
                !backend.snapshot_save(slot)) {
                std::printf("err bad_snapshot\n");
                std::fflush(stdout);
                continue;
            }
        ...
        if (starts_with(line, "FREE_SNAPSHOT ")) {
            int slot = -1;
            if (!parse_int_arg(line.c_str() + 14, slot) ||
                slot < 0 || slot >= ModelBackend::kMaxSlots) {
                std::printf("err bad_snapshot\n");
                std::fflush(stdout);
                continue;
