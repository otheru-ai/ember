...
inline bool GgufMmap::open(const std::string & path, std::string & out_error) {
    ...
    if (MultiByteToWideChar(
            CP_UTF8, 0, path.c_str(), -1, wpath.data(), wlen) != wlen) {
        out_error = "GgufMmap: UTF-8 path conversion failed for " + path;
        return false;
    }
    ...
    if (li.QuadPart <= 0 ||
        ...
        out_error = "GgufMmap: invalid file size for " + path;
        CloseHandle(hFile);
        return false;
    }
...
inline void GgufMmap::advise_willneed(size_t offset, size_t length) const {
    ...
    const size_t aligned_length =
        length > SIZE_MAX - prefix ? size_ - aligned_offset : length + prefix;
