...
static std::mutex g_peer_cache_mu;

// ── helpers ─────────────────────────────────────────────────────
bool enable_peer_access_one_way(int device, int peer) {
    if (device < 0 || peer < 0) return false;
...
bool cross_device_peer_memcpy_ok(int src_device, int dst_device) {
    ...
    std::lock_guard<std::mutex> lock(g_peer_cache_mu);
...
bool copy_peer_async(void * dst, int dst_device,
    ...
    if (!dst || !src || dst_device < 0 || src_device < 0) return false;
