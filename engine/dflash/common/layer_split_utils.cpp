    ...
    const std::vector<double> & weights)
{
    ...
    double sum = 0.0;
    for (double v : w) {
        if (!std::isfinite(v) || v < 0.0) return ranges;
        sum += v;
        if (!std::isfinite(sum)) return ranges;
    }
...
bool init_layer_split_shard_metas(
    ...
    for (size_t i = 0; i < shards.size(); ++i) {
        if (!shards[i] || shards[i]->backend || gpus[i] < 0 ||
            ranges[i].begin < 0 || ranges[i].end <= ranges[i].begin ||
            (i > 0 && ranges[i - 1].end != ranges[i].begin)) {
            return false;
        }
    }
    auto rollback = [&](size_t initialized) {
        for (size_t j = 0; j < initialized; ++j) {
            if (shards[j] && shards[j]->backend) {
                ggml_backend_free(shards[j]->backend);
                shards[j]->backend = nullptr;
            }
        }
    };
    for (size_t i = 0; i < shards.size(); ++i) {
