...
__device__ __forceinline__ float softmax_weight(
        float value, float inv_t, float xmax, float max_logit) {
    if (isnan(value)) return 0.0f;
    if (isinf(max_logit) && max_logit > 0.0f)
        return isinf(value) && value > 0.0f ? 1.0f : 0.0f;
    return expf(value * inv_t - xmax);
}
...
__global__ void geometric_sample_kernel(float * __restrict__ work, int vocab,
    ...
    for (int v = t; v < nvec; v += nthreads) {
        const float4 f    = w4[v];
        const int    base = v << 2;
        const float x = isnan(f.x) ? -INFINITY : f.x;
        const float y = isnan(f.y) ? -INFINITY : f.y;
        const float z = isnan(f.z) ? -INFINITY : f.z;
        const float w = isnan(f.w) ? -INFINITY : f.w;
        if (x > lmax || (x == lmax && base     < largmax)) { lmax = x; largmax = base;     }
        if (y > lmax || (y == lmax && base + 1 < largmax)) { lmax = y; largmax = base + 1; }
        if (z > lmax || (z == lmax && base + 2 < largmax)) { lmax = z; largmax = base + 2; }
        if (w > lmax || (w == lmax && base + 3 < largmax)) { lmax = w; largmax = base + 3; }
    }
    for (int i = tail + t; i < vocab; i += nthreads) {
        const float v = isnan(work[i]) ? -INFINITY : work[i];
        if (v > lmax || (v == lmax && i < largmax)) {
            lmax = v;
            largmax = i;
        }
    }
    ...
    if (!isfinite(lmax) && lmax < 0.0f) {
        if (mode == kModeEmitProbs) {
            for (int i = t; i < vocab; i += nthreads)
                out_probs[i] = i == argmax ? 1.0f : 0.0f;
        } else if (t == 0) {
            *out_token = argmax;
        }
        return;
    ...
    for (int v = t; v < nvec; v += nthreads) {
        const float4 f = w4[v];
        lz += softmax_weight(f.x, inv_t, xmax, lmax) +
              softmax_weight(f.y, inv_t, xmax, lmax) +
              softmax_weight(f.z, inv_t, xmax, lmax) +
              softmax_weight(f.w, inv_t, xmax, lmax);
    }
    for (int i = tail + t; i < vocab; i += nthreads)
        lz += softmax_weight(work[i], inv_t, xmax, lmax);
    ...
    if (mode == kModeEmitProbs) {
        ...
        for (int v = t; v < nvec; v += nthreads) {
            const float4 f = w4[v];
            float4 o;
            o.x = softmax_weight(f.x, inv_t, xmax, lmax) * inv_z;
            o.y = softmax_weight(f.y, inv_t, xmax, lmax) * inv_z;
            o.z = softmax_weight(f.z, inv_t, xmax, lmax) * inv_z;
            o.w = softmax_weight(f.w, inv_t, xmax, lmax) * inv_z;
            o4[v] = o;
        }
        for (int i = tail + t; i < vocab; i += nthreads)
            out_probs[i] = softmax_weight(work[i], inv_t, xmax, lmax) * inv_z;
        return;
    }
    ...
    if (targetv >= (double)s_off[t] && targetv < (double)s_off[t] + pm) {
        ...
        for (int v = t; v < nvec; v += nthreads) {
            const float4 f    = w4[v];
            const int    base = v << 2;
            acc += softmax_weight(f.x, inv_t, xmax, lmax); if (targetv < acc) { *out_token = base;     goto done; }
            acc += softmax_weight(f.y, inv_t, xmax, lmax); if (targetv < acc) { *out_token = base + 1; goto done; }
            acc += softmax_weight(f.z, inv_t, xmax, lmax); if (targetv < acc) { *out_token = base + 2; goto done; }
            acc += softmax_weight(f.w, inv_t, xmax, lmax); if (targetv < acc) { *out_token = base + 3; goto done; }
        }
        for (int i = tail + t; i < vocab; i += nthreads) {
            acc += softmax_weight(work[i], inv_t, xmax, lmax);
...
void free_scratch() {
    ...
    if (g_scratch.device >= 0 && g_scratch.device != previous_device)
        (void) cudaSetDevice(g_scratch.device);
...
bool stage_and_penalize(int dev, const float * logits, int vocab, const SamplerCfg & cfg,
    ...
    if ((rep_active || add_active) && !history.empty()) {
        ...
        for (const auto & kv : counts) {
            ...
            pen_add.push_back(add_active
                ? (cfg.freq_pen * static_cast<float>(kv.second) + cfg.pres_pen)
                : 0.0f);
        }
    }
...
int geometric_sample_logits_cuda(const float * logits,
    ...
    if (!logits || vocab <= 0 || !std::isfinite(cfg.temp) ||
        !std::isfinite(cfg.rep_pen) || cfg.rep_pen <= 0.0f ||
        !std::isfinite(cfg.freq_pen) || !std::isfinite(cfg.pres_pen) ||
...
bool geometric_compute_probs_cuda(const float * logits,
                        ...
                        float * out_probs,
                        bool logits_on_device) {
    if (!logits || vocab <= 0 || !out_probs || cfg.temp <= 0.0f ||
        !std::isfinite(cfg.temp) || !std::isfinite(cfg.rep_pen) ||
        cfg.rep_pen <= 0.0f || !std::isfinite(cfg.freq_pen) ||
        !std::isfinite(cfg.pres_pen)) return false;
