...
bool placement_valid(const MoeHybridPlacement & p, std::string * err) {
    ...
    for (int il = 0; il < p.n_layer; ++il) {
        ...
        if (ids.size() > static_cast<size_t>(p.n_expert) ||
            ...
            if (err) *err = "hot count does not match expert list";
            return false;
        }
        std::fill(seen.begin(), seen.end(), 0);
        for (int32_t id : ids) {
            if (id < 0 || id >= p.n_expert || seen[static_cast<size_t>(id)]) {
                if (err) *err = "hot expert IDs must be unique and in range";
                return false;
            }
    ...
    if (total > std::numeric_limits<int>::max() ||
        ...
        if (err) *err = "total_hot does not match expert lists";
        return false;
    }
    return true;
}
...
bool MoeHybridPlacement::save_json(const std::string & path, const std::string & arch_name,
    ...
    if (!placement_valid(*this, err)) return false;
...
bool MoeHybridPlacement::load_json(const std::string & path,
    ...
    if (!placement_valid(tmp, err)) return false;
...
bool MoeHybridPlacement::build_from_stats(const MoeHybridRoutingStats & stats,
    ...
    const int64_t floor_total =
        static_cast<int64_t>(per_layer_floor) * stats.n_layer;
    ...
    int64_t remaining = static_cast<int64_t>(total_hot_budget) - floor_total;
    while (remaining > 0) {
        int best_layer = -1;
...
bool MoeHybridPlacement::build_from_stats_with_layer_bytes(
    ...
    for (int il = 0; il < stats.n_layer; ++il) {
        if (layer_expert_bytes[(size_t)il] > 0) {
            ...
            if (static_cast<uint64_t>(per_layer_floor) >
                    std::numeric_limits<uint64_t>::max() / bytes ||
                floor_bytes >
                    std::numeric_limits<uint64_t>::max() -
                    static_cast<uint64_t>(per_layer_floor) * bytes) {
                if (err) *err = "minimum hot placement byte count overflows";
                return false;
            }
            floor_bytes += static_cast<uint64_t>(per_layer_floor) * bytes;
        }
    }
