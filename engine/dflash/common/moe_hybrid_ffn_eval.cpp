...
static int env_int_or_default(const char * name, int fallback) {
    ...
    if (errno == ERANGE || end == raw || *end != '\0') return fallback;
    if (value < 1) return 1;
...
static int fixed_slot_max() {
    static const int max_slots = [] {
        ...
        if (errno == ERANGE || end == env || *end != '\0' || value <= 0) return 0;
        return static_cast<int>(std::min<long>(value, 4096));
    }();
    return max_slots;
}

static bool validate_moe_cfg(const MoeHybridConfig & cfg, std::string * err) {
    if (cfg.n_embd <= 0 || cfg.n_expert <= 0 || cfg.n_expert_used <= 0 ||
        cfg.n_expert_used > cfg.n_expert || cfg.n_ff_exp <= 0 ||
        !std::isfinite(cfg.swiglu_clamp) || cfg.swiglu_clamp < 0.0f) {
        if (err) *err = "invalid MoE dimensions or clamp";
        return false;
    }
    return true;
}

static bool validate_moe_batch_inputs(
        ...
        int n_tokens,
        std::string * err) {
    if (!validate_moe_cfg(cfg, err)) return false;
    if (n_tokens < 0) {
        if (err) *err = "negative MoE token count";
        return false;
    }
    if (n_tokens == 0) return true;
    if (!cur_host || !selected_ids || !selected_weights ||
        static_cast<size_t>(n_tokens) >
            std::numeric_limits<size_t>::max() / static_cast<size_t>(cfg.n_embd) ||
        static_cast<size_t>(n_tokens) >
            std::numeric_limits<size_t>::max() / static_cast<size_t>(cfg.n_expert_used) ||
        n_tokens > INT_MAX / cfg.n_expert_used) {
        if (err) *err = "invalid or oversized MoE batch inputs";
        return false;
    }
    const size_t n_slots =
        static_cast<size_t>(n_tokens) * static_cast<size_t>(cfg.n_expert_used);
    for (size_t i = 0; i < n_slots; ++i) {
        if (selected_ids[i] < 0 || selected_ids[i] >= cfg.n_expert ||
            !std::isfinite(selected_weights[i])) {
            if (err) *err = "invalid MoE expert id or weight";
            return false;
        }
    }
    return true;
...
bool eval_moe_hybrid_ffn_single(
    ...
    if (!gpu_backend || !cpu_backend || !validate_moe_cfg(cfg, err) ||
        !cur_host || n_selected < 0 || n_selected > cfg.n_expert_used ||
        (n_selected > 0 && (!selected_ids || !selected_weights)) ||
        storage.hot_local_by_global.size() < static_cast<size_t>(cfg.n_expert) ||
        storage.cold_local_by_global.size() < static_cast<size_t>(cfg.n_expert)) {
        if (err && err->empty()) *err = "invalid single-token MoE inputs";
        return false;
    }
    for (int i = 0; i < n_selected; ++i) {
        if (!std::isfinite(selected_weights[i])) {
            if (err) *err = "non-finite MoE expert weight";
            return false;
        }
    }
...
// illegal-accesses on a reduced expert stack for sparse/imbalanced batches on
// gfx1151. Default to the proven single-token path; the env override permits
// measured tuning without a rebuild.
static int mmq_safe_sub_batch() {
    static const int v = [](){
        const char * e = std::getenv("DFLASH_MMQ_SUB_BATCH");
        if (e && *e) return env_int_or_default("DFLASH_MMQ_SUB_BATCH", 1);
        return 1;
    }();
    return v;
}
...
int moe_hybrid_prefill_hot_sub_batch_limit() {
    ...
    if (raw && *raw) {
        ...
        if (errno != ERANGE && end != raw && *end == '\0' &&
            ...
            requested = value > 4096 ? 4096 : static_cast<int>(value);
        }
    }
    if (requested <= 0) {
...
static bool eval_moe_hybrid_ffn_batched_core(
    ...
    if (n_tokens > 0 && n_tokens < MoeHybridLayerStorage::kMaxBatchedCache) {
        ...
        const int n_hot_init = std::max(1, (int)storage.hot_expert_ids.size());
...
bool eval_moe_hot_only_batched(
    ...
    out.clear();
    if (!gpu_backend ||
        !validate_moe_batch_inputs(
            cfg, cur_host, selected_ids, selected_weights, n_tokens, err) ||
        storage.hot_local_by_global.size() < static_cast<size_t>(cfg.n_expert)) {
        if (err && err->empty()) *err = "invalid hot-only MoE inputs";
        return false;
    }
    if (n_tokens == 0) return true;
    const int n_embd = cfg.n_embd;
    ...
    if (n_hot_stack > 0 && n_tokens > MMQ_SAFE_SUB_BATCH) {
    ...
    for (int i = 0; i < total_slots; ++i) {
        const int32_t gid = selected_ids[i];
        const int32_t local = storage.hot_local_by_global[(size_t)gid];
        if (local < 0) {
            if (err) *err = "hot-only batch selected a nonresident expert";
            return false;
        }
        hot_sel[i] = local;
    }
    ...
    // Cache the proven sub-batch size or a full expert stack.
    if (n_tokens <= MMQ_SAFE_SUB_BATCH ||
...
bool eval_moe_hybrid_ffn_batched(
    ...
    if (!gpu_backend || !cpu_backend ||
        ...
        storage.hot_local_by_global.size() < static_cast<size_t>(cfg.n_expert) ||
        storage.cold_local_by_global.size() < static_cast<size_t>(cfg.n_expert)) {
        if (err && err->empty()) *err = "invalid hybrid MoE batch inputs";
        return false;
    }
    if (n_tokens == 0) return true;
    const int n_hot_stack = storage.gate_up_hot ? (int)storage.gate_up_hot->ne[2]
    ...
    if (((n_hot_stack > 0 && n_hot_stack < cfg.n_expert) ||
