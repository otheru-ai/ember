...
bool kv_i32_array(gguf_context * g, const std::string & key, std::vector<int> & out) {
    ...
    if (at == GGUF_TYPE_INT32) {
        ...
        for (size_t i = 0; i < n; i++) {
            if (v[i] > static_cast<uint32_t>(std::numeric_limits<int>::max())) {
                out.clear();
                return false;
            }
...
bool load_deepseek4_dspark_drafter(const std::string & path,
    ...
    // ── Arch check ──────────────────────────────────────────────────────
    {
        ...
        if (gguf_get_kv_type(g, aid) != GGUF_TYPE_STRING) {
            set_err("general.architecture must be a string");
            gguf_free(g);
            if (meta) ggml_free(meta);
            return false;
        }
        const char * arch = gguf_get_val_str(g, aid);
        if (!arch || std::strcmp(arch, ARCH) != 0) {
            set_err(std::string("unexpected arch: ") +
                    (arch ? arch : "(null)") + " (expected " + ARCH + ")");
