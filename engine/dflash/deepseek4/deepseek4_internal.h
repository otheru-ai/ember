...
// Compressed-row block granularity published to the D=512 flash kernel. It is
// the unit of both its optional block-sparse selection and its shared-memory
// block bookkeeping, so the graph and the eligibility estimate must agree.
inline constexpr int DS4_FLASH_COMP_BLOCK = 32;
// gfx1151 exposes 64 KiB of LDS per workgroup. Reserve a little of it for the
// kernel's static shared state and launch overhead rather than sizing decode
// attention exactly to the hardware maximum.
inline constexpr std::size_t DS4_DECODE_FLASH_SHMEM_BUDGET = 63u * 1024u;
...
// `host_logits` / `host_spec_feat` are optional host-side state that the
// tensor snapshot alone does not carry (last prompt logits, DSpark rolling
// target-feature window). Pass them when the snapshot may be serialized to
// disk: on reload there is no live backend state to recover them from, and
// restore_and_generate_impl rejects a slot whose logits are empty.
bool deepseek4_snapshot_save(const DeepSeek4Cache & cache,
                             ggml_backend_t snapshot_backend,
                             DeepSeek4Snapshot & out,
                             const std::vector<float> * host_logits = nullptr,
                             const std::vector<float> * host_spec_feat = nullptr);
...
// True when a full-model batch can use the device-resident layer-major
// prefill scheduler. DSpark capture batches deliberately stay on the
// compressor-boundary path because the layer-major graph does not emit
// verifier capture hooks.
...
struct Ds4VerifyHooks {
    ...
    // Exact DSpark q=1 capture must use the ordinary fused target graph. A
    // fallback to the hook-aware per-layer graph can change near-tied logits.
    bool require_fused_q1 = false;
};
...
struct DeepSeek4Snapshot {
    ...
    // Serialization side-channel. The per-layer n_comp / n_index_comp counters
    // and the host-side logits are not tensors, so a snapshot round-tripped
    // through disk would lose them. Carrying them as tensors inside `ctx`
    // means the existing tensor-based disk format persists them for free.
    ggml_tensor *         meta_snap      = nullptr;  // I32 [2 * n_layer]
    ggml_tensor *         logits_snap    = nullptr;  // F32 [vocab], may be null
    ggml_tensor *         spec_feat_snap = nullptr;  // F32 [n], may be null
    ggml_context *        ctx = nullptr;
    ggml_backend_buffer_t buf = nullptr;
};
