...
// Attention implementation selected by the DS4 prefill scheduler. Decode is not
// scheduled through it: q=1 stays on the explicit reduction unless the fused
// full-ring path opts into flash attention (deepseek4_decode_flash_eligible).
...
// Mirror ggml_new_tensor_impl's view bound calculation before constructing a
// raw-KV row view.  ggml's invariant is process-fatal; returning nullptr here
// lets the existing graph-build -> prefill-failure path reject only this
// request while preserving the exact metadata needed to diagnose the fault.
//
// The final reshape in build_tail_rope_2d() already checked the same base
// allocation.  Capture that state at creation and print it beside the state at
// the row write: a difference localizes corruption to the intervening graph
// construction, while identical values would expose a mismatch in this mirror.
struct Ds4RawKvViewState {
    const ggml_tensor * tensor = nullptr;
    const ggml_tensor * view_src = nullptr;
    int type = -1;
    int64_t ne[GGML_MAX_DIMS] = {};
    size_t nb[GGML_MAX_DIMS] = {};
    size_t view_offs = 0;
    size_t relative_offs = 0;
    size_t absolute_offs = 0;
    size_t requested_bytes = 0;
    size_t base_nbytes = 0;
    bool valid = false;
};

static Ds4RawKvViewState ds4_raw_kv_view_state(
        const ggml_tensor * tensor,
        int64_t row_width,
        int row,
        size_t row_stride) {
    Ds4RawKvViewState state;
    state.tensor = tensor;
    if (!tensor || row_width <= 0 || row < 0 ||
        tensor->type < 0 || tensor->type >= GGML_TYPE_COUNT) {
        return state;
    }
    // ggml collapses one view level: a view is checked against its source;
    // a non-view is itself the source allocation for the requested row view.
    state.view_src = tensor->view_src ? tensor->view_src : tensor;
    state.type = (int) tensor->type;
    for (int i = 0; i < GGML_MAX_DIMS; ++i) {
        state.ne[i] = tensor->ne[i];
        state.nb[i] = tensor->nb[i];
    }
    state.view_offs = tensor->view_src ? tensor->view_offs : 0;
    if (row > 0 && row_stride > SIZE_MAX / (size_t) row) return state;
    state.relative_offs = (size_t) row * row_stride;
    state.requested_bytes = ggml_row_size(tensor->type, row_width);

    state.base_nbytes = ggml_nbytes(state.view_src);
    const DeepSeek4ViewBound bound = deepseek4_view_bound(
        state.relative_offs, state.view_offs, state.requested_bytes,
        state.base_nbytes);
    state.absolute_offs = bound.absolute_offs;
    state.valid = bound.valid;
    return state;
}

static void ds4_log_invalid_raw_kv_view(
        const Ds4RawKvViewState & created,
        const Ds4RawKvViewState & current,
        int layer_idx,
        int kv_start,
        int n_tokens,
        int ti,
        ...
        int ratio) {
    std::fprintf(stderr,
        "[deepseek4] invalid raw-KV row view: layer=%d kv_start=%d "
        "n_tokens=%d ti=%d head_dim=%d ratio=%d\n"
        "  current: tensor=%p type=%d ne=[%lld,%lld,%lld,%lld] "
        "nb=[%zu,%zu,%zu,%zu] view_src=%p view_offs=%zu "
        "relative_offs=%zu absolute_offs=%zu requested=%zu base_nbytes=%zu\n"
        "  created: tensor=%p type=%d ne=[%lld,%lld,%lld,%lld] "
        "nb=[%zu,%zu,%zu,%zu] view_src=%p view_offs=%zu "
        "absolute_offs=%zu requested=%zu base_nbytes=%zu valid=%d\n",
        layer_idx, kv_start, n_tokens, ti, head_dim, ratio,
        (const void *) current.tensor, current.type,
        (long long) current.ne[0], (long long) current.ne[1],
        (long long) current.ne[2], (long long) current.ne[3],
        current.nb[0], current.nb[1], current.nb[2], current.nb[3],
        (const void *) current.view_src, current.view_offs,
        current.relative_offs, current.absolute_offs,
        current.requested_bytes, current.base_nbytes,
        (const void *) created.tensor, created.type,
        (long long) created.ne[0], (long long) created.ne[1],
        (long long) created.ne[2], (long long) created.ne[3],
        created.nb[0], created.nb[1], created.nb[2], created.nb[3],
        (const void *) created.view_src, created.view_offs,
        created.absolute_offs, created.requested_bytes,
        created.base_nbytes, created.valid ? 1 : 0);
}
...
bool deepseek4_layer_major_prefill_candidate(
        int n_tokens,
        int layer_begin,
        bool is_last_shard,
        bool has_out_logits,
        bool is_gpu,
        bool moe_hybrid,
        bool has_verify_hooks,
        PrefillAttentionMode prefill_mode) {
    return n_tokens > 4 &&
           ...
           !moe_hybrid &&
           !has_verify_hooks &&
           prefill_mode != PrefillAttentionMode::Exact;
}
...
static ggml_tensor * build_mla_attention(
    ...
    // access and a candidate for the SIGSEGV seen in production on the
    // spec-decode + snapshot-restore path.
    ds4_bc_mla(layer_idx, kv_start, n_tokens,
               (int) w.compress_ratios.size(), (int) attention_impl,
               w.n_layer, lc.n_comp);
    ...
    kv = build_tail_rope_2d(ctx, kv, rope_pos, n_rot, head_dim, n_tokens,
                            rope_freq, rope_scale, rope_ext, rope_attn,
                            w.rope_yarn_beta_fast, w.rope_yarn_beta_slow, rope_n_ctx_orig);
    const Ds4RawKvViewState kv_view_created =
        ds4_raw_kv_view_state(kv, head_dim, 0, kv->nb[1]);
    if (!kv_view_created.valid) {
        ds4_log_invalid_raw_kv_view(
            kv_view_created, kv_view_created, layer_idx, kv_start,
            n_tokens, 0, head_dim, ratio);
        return nullptr;
    }
    ...
    const DeepSeek4AttentionShape attention_shape = deepseek4_attention_shape(
        n_tokens, w.n_swa, cached_inputs != nullptr,
        f32_array_inputs != nullptr,
        !ds4_env_flag("DFLASH_DS4_NO_CAUSAL_VERIFY"),
        attention_impl != DeepSeek4AttentionImpl::Explicit,
        cached_inputs && cached_inputs->attn_row_mask);
    ...
    int n_prior_rows = 0;
    const bool fused_causal = attention_shape.fused_causal;
    if (fused_causal) {
    ...
    if (raw_kv_rows) {
        ...
        const int first_write = attention_shape.first_raw_kv_write;
        for (int ti = first_write; ti < n_tokens; ti++) {
            const int pos_ti = kv_start + ti;
            const Ds4RawKvViewState kv_view_current =
                ds4_raw_kv_view_state(kv, head_dim, ti, kv->nb[1]);
            ds4_bc_kv_view(
                layer_idx, ti, kv_view_current.type,
                (uint64_t)(uintptr_t) kv_view_current.tensor,
                (uint64_t)(uintptr_t) kv_view_current.view_src,
                (uint64_t) kv_view_current.ne[0],
                (uint64_t) kv_view_current.ne[1],
                (uint64_t) kv_view_current.nb[0],
                (uint64_t) kv_view_current.nb[1],
                (uint64_t) kv_view_current.view_offs,
                (uint64_t) kv_view_current.relative_offs,
                (uint64_t) kv_view_current.absolute_offs,
                (uint64_t) kv_view_current.requested_bytes,
                (uint64_t) kv_view_current.base_nbytes,
                (uint64_t) kv_view_created.view_offs,
                (uint64_t) kv_view_created.base_nbytes);
            if (!kv_view_current.valid) {
                ds4_log_invalid_raw_kv_view(
                    kv_view_created, kv_view_current, layer_idx, kv_start,
                    n_tokens, ti, head_dim, ratio);
                return nullptr;
            }
    ...
    // Decode can run the same exact D=512 kernel prefill already uses, but only
    // on the fused full-ring path: the mask is the sole visibility authority
    // there, so slot order carries no meaning the kernel would have to infer.
    // Opt-in while the numerics are still being compared against the explicit
    // reduction below (README: DFLASH_DS4_DECODE_FLASH).
    const bool decode_flash =
        deepseek4_decode_flash_eligible(
            n_tokens, head_dim, n_attn, n_comp_attn, masked_kv,
            DS4_FLASH_COMP_BLOCK, DS4_DECODE_FLASH_SHMEM_BUDGET, n_head) &&
        ds4_env_flag("DFLASH_DS4_DECODE_FLASH");
    ...
    if (masked_kv) {
        ...
        // The decode WMMA kernel consumes K as F16, which is its storage type,
        // so the F32 round-trip is pure overhead there: the cast is lossless
        // (values originate in F16) and skipping it halves the K read.
        kv_attn = decode_flash
            ? ring
            : ds4_cast_if_needed(ctx, ring, GGML_TYPE_F32);
    } else if (layer_major_batch) {
    ...
    const bool use_flash =
        (attention_impl != DeepSeek4AttentionImpl::Explicit && n_tokens > 1) ||
        decode_flash;
    if (use_flash) {
        if (exact_two_band) {
            ...
            auto make_flash = [&](ggml_tensor * q_band,
                ...
                ggml_flash_attn_ext_set_ds4_sparse(
                    result, raw_count, w.n_swa, 0, DS4_FLASH_COMP_BLOCK);
            ...
            // Decode keeps every compressed row. The explicit reduction it
            // replaces is dense, and the full-ring path pads the compressed
            // span, so a block mean would average live rows with the padding
            // this mask exists to hide.
            const int sparse_keep_rows = indexer_topk
                ? -w.n_indexer_top_k
                : (!decode_flash &&
            ...
            ggml_flash_attn_ext_set_ds4_sparse(
                context, n_raw, w.n_swa, sparse_keep_rows,
                DS4_FLASH_COMP_BLOCK);
            // Fusing the inverse RoPE bakes kv_start into the node's op params.
            // The cached q=1 graph is reused at every later position, so decode
            // keeps the tail rotation as a graph node reading the per-step
            // neg_pos input instead.
...
bool deepseek4_step(
    ...
    return deepseek4_step_layer_range(
        ...
        /*allow_decode_graph_reuse=*/verify_hooks == nullptr || n_tokens == 1,
        verify_hooks);
}
...
struct DeepSeek4FusedDecodeGraph {
    ...
    ggml_tensor * terminal = nullptr;      // logits or final HC state
    ggml_tensor * logits = nullptr;
    ggml_tensor * capture = nullptr;       // optional passive q=1 DSpark features
...
static bool ds4_build_fused_decode_graph(
    ...
    std::vector<ggml_tensor *> capture_pieces(
        capture_ids ? capture_ids->size() : 0, nullptr);
    ...
    for (int il = 0; il < w.n_layer; ++il) {
        ...
        // DSpark consumes the mean HC state after the FFN at its target
        // layers.  Make those states additional graph outputs instead of
        // selecting the hook-aware per-layer verifier graph.  The ordinary
        // logits graph is expanded first below, so these read-only reductions
        // are appended without changing any target operation or its order.
        if (capture_ids) {
            for (size_t ci = 0; ci < capture_ids->size(); ++ci) {
                if ((*capture_ids)[ci] != il) continue;
                ggml_tensor * hs_t = ggml_cont(ctx, ggml_transpose(ctx, hc_cur));
                ggml_tensor * summed = ggml_sum_rows(ctx, hs_t);
                ggml_tensor * mean = ggml_scale(
                    ctx, summed, 1.0f / (float) n_hc);
                capture_pieces[ci] = ggml_reshape_1d(
                    ctx, ggml_cont(ctx, mean), n_embd);
            }
        }
    }

    // Intermediate q=1 prompt tokens only need to publish their attention and
    // compressor state.  The final HC merge and 129k-vocabulary projection do
    // not feed any subsequent token, so omitting them is exact: every
    // stateful operation and its reduction order above remains unchanged.
    ggml_tensor * hc_flat = ggml_reshape_1d(ctx, hc_cur, (int64_t) n_embd * n_hc);
    if (need_logits) {
        ...
        ggml_tensor * final_embd = ggml_ds4_hc_out(
            ctx, omix, obase, hc_flat, n_hc,
            hc_out_weights.scale_data[0]);
        ggml_tensor * final_2d = ggml_reshape_2d(
            ctx, final_embd, n_embd, 1);
        ggml_tensor * out_normed = build_rms_norm(
            ctx, final_2d, w.out_norm, w.rms_eps);
        fg.logits = ggml_mul_mat(ctx, w.output, out_normed);
        fg.terminal = fg.logits;
    } else {
        fg.terminal = hc_cur;
    }
    ggml_set_output(fg.terminal);
    ggml_build_forward_expand(gf, fg.terminal);

    if (!capture_pieces.empty()) {
        for (ggml_tensor * piece : capture_pieces) {
            if (!piece) {
                std::fprintf(stderr,
                             "[deepseek4] fused decode: invalid DSpark capture layer\n");
                return false;
            }
        }
        fg.capture = capture_pieces[0];
        for (size_t i = 1; i < capture_pieces.size(); ++i) {
            fg.capture = ggml_concat(ctx, fg.capture, capture_pieces[i], 0);
        }
        ggml_set_output(fg.capture);
        ggml_build_forward_expand(gf, fg.capture);
    }
...
// failure after cache state may have been touched. out_logits may be null for
// state-only q=1 prefill steps.
static int ds4_try_fused_decode_step(
    ...
    const int token_pos = kv_start;
    const bool need_logits = out_logits || (hooks && hooks->all_logits_out);
    std::vector<int64_t> key;
    const std::vector<int> * capture_ids = hooks
        ? hooks->capture_layer_ids
        : nullptr;
    key.reserve((size_t) w.n_layer + 4 +
                (capture_ids ? capture_ids->size() : 0));
    key.push_back(w.n_swa);
    key.push_back(token_ids ? 1 : 0);
    key.push_back(need_logits ? 1 : 0);
    key.push_back(capture_ids ? (int64_t) capture_ids->size() : 0);
    if (capture_ids) {
    ...
    if (!fg) {
        ...
        if (!ds4_build_fused_decode_graph(fc, *fg, backend, w, cache,
                                          ...
                                          need_logits, capture_ids,
                                          std::move(key))) {
            std::fprintf(stderr,
                         "[deepseek4] fused decode graph build failed; using per-layer path\n");
            step_graph_free(fg->sg);
            fg->reset_nodes();
            // A capture-only graph may fail because of an invalid drafter
            // contract or its extra allocation. Do not poison ordinary AR
            // fused decode globally; exact DSpark will fail closed below.
            if (!capture_ids) fc.disabled = true;
            return 0;
        }
    ...
    if (need_logits) {
        if (!fg->logits) {
            std::fprintf(stderr,
                         "[deepseek4] fused decode: logits output missing\n");
            return -1;
        }
        std::vector<float> hook_logits;
        std::vector<float> & logits = out_logits ? *out_logits : hook_logits;
        logits.resize((size_t) w.n_vocab);
        ggml_backend_tensor_get(fg->logits, logits.data(), 0,
                                sizeof(float) * (size_t) w.n_vocab);
        if (hooks && hooks->all_logits_out) {
            *hooks->all_logits_out = logits;
        }
    }
    if (hooks && hooks->capture_out && capture_ids &&
        !capture_ids->empty()) {
        if (!fg->capture) {
            std::fprintf(stderr,
                         "[deepseek4] fused decode: capture output missing\n");
            return -1;
        }
        hooks->capture_out->resize(
            capture_ids->size() * (size_t) w.n_embd);
        ggml_backend_tensor_get(
            fg->capture, hooks->capture_out->data(), 0,
...
struct Ds4LayerMajorGraphCache {
    ...
    // #5: RAII teardown so thread exit (or std::array destruction) reclaims the
    // ggml context, backend buffer, and every per-layer ctx/meta_buffer this
    // cache owns, even when deepseek4_release_runtime_graphs() was never called.
    // destroy() nulls each handle after freeing it (and clears `layers`), so a
    // second run from an explicit release path is a safe no-op (no double free).
...
// #5: Thread-exit guard for the shared gallocr. The thread_local gallocr above
// is a raw handle with no destructor, so a worker thread that built a
// layer-major graph and then exited without calling
// deepseek4_release_runtime_graphs() would leak its gallocr arena. This guard's
// destructor runs at thread exit and reclaims it. It is idempotent: once the
// arena has been freed and the handle nulled (here, or by an explicit release),
// it no-ops. arm() is called from ds4_layer_major_get_shared_alloc() whenever a
// gallocr is created; that odr-use is what registers this thread_local's
// destructor for the exiting thread.
struct Ds4LayerMajorSharedAllocGuard {
    void arm() const {}
    ~Ds4LayerMajorSharedAllocGuard() {
...
static thread_local Ds4LayerMajorSharedAllocGuard ds4_layer_major_shared_alloc_guard;
...
static ggml_gallocr_t ds4_layer_major_get_shared_alloc(
    ...
    if (!ds4_layer_major_shared_alloc) {
        ...
        ds4_layer_major_shared_owner = w.ctx;
        ds4_layer_major_shared_backend = backend;
        // #5: register the thread-exit guard so this gallocr is reclaimed at
        // thread exit even if deepseek4_release_runtime_graphs() is not called.
        ds4_layer_major_shared_alloc_guard.arm();
    }
...
static int ds4_try_layer_major_prefill(
    ...
    const bool ds4_gtt_trace = std::getenv("EMBER_GTT_TRACE") != nullptr;
    auto GTT = [&](const char * tag) {
        if (!ds4_gtt_trace) return;
        long mb = -1;
        if (FILE * gfp = std::fopen("/sys/class/drm/card1/device/mem_info_gtt_used", "r")) {
            long long bb = 0; if (std::fscanf(gfp, "%lld", &bb) == 1) mb = (long)(bb / (1024 * 1024));
            std::fclose(gfp);
        }
        std::fprintf(stderr, "[gtt] %-20s n=%d kv=%d hit=%d build=%d gtt=%ldMB\n",
                     tag, n_tokens, kv_start, (int) cache_hit, (int) cache_build, mb);
    };
    GTT("cache_decided");
    ...
    GTT("before_tail_finalize");
    if (ds4_gtt_trace) std::fprintf(stderr, "[gtt] arena_buf=%zuMB n=%d kv=%d build=%d\n", ggml_gallocr_get_buffer_size(alloc, 0) / (1024 * 1024), n_tokens, kv_start, (int) cache_build);
    if (cache_build) {
...
bool deepseek4_step_layer_range(
    ...
    // Reject malformed runtime state before any vector indexing or graph
    // construction. Snapshot restore and distributed layer ranges both enter
    // here, so a stale/incompatible cache must fail the request rather than
    // turn an invalid layer reference into a process-wide SIGSEGV.
    if (!backend || !embed || n_tokens <= 0 || kv_start < 0 ||
        layer_begin < 0 || layer_end <= layer_begin ||
        layer_end > w.n_layer ||
        w.layers.size() != (size_t) w.n_layer ||
        w.compress_ratios.size() != (size_t) w.n_layer ||
        cache.n_layer != w.n_layer ||
        cache.layers.size() != (size_t) cache.n_layer ||
        (cache.max_ctx > 0 &&
         ((int64_t) kv_start + n_tokens > cache.max_ctx))) {
        std::fprintf(stderr,
                     "[deepseek4] invalid step state: tokens=%d kv=%d "
                     "layers=[%d,%d)/%d weights=%zu ratios=%zu "
                     "cache=%d/%zu max_ctx=%d\n",
                     n_tokens, kv_start, layer_begin, layer_end, w.n_layer,
                     w.layers.size(), w.compress_ratios.size(),
                     cache.n_layer, cache.layers.size(), cache.max_ctx);
        return false;
    }
    for (int il = layer_begin; il < layer_end; ++il) {
        const DeepSeek4LayerCache & lc = cache.layers[(size_t) il];
        const uint32_t ratio = w.compress_ratios[(size_t) il];
        if (!lc.raw_kv ||
            (ratio > 0 &&
             (!lc.comp_kv || !lc.attn_compressor.state_kv ||
              !lc.attn_compressor.state_score)) ||
            (ratio == 4 &&
             (!lc.index_comp_kv || !lc.indexer_compressor.state_kv ||
              !lc.indexer_compressor.state_score))) {
            std::fprintf(stderr,
                         "[deepseek4] incomplete KV state for layer %d "
                         "(ratio=%u)\n",
                         il, ratio);
            return false;
        }
    ...
    // The layer-major graph implements its own exact batched compressor
    ...
    // q4 into smaller dynamic forwards and makes the fused path unreachable.
    //
    // Keep hybrid placement excluded explicitly. DeepSeek4Backend currently
    ...
    if (!layer_major_candidate && !fused_verify_candidate &&
        first_chunk > 0 && first_chunk < n_tokens) {
        ...
        for (int off = 0; off < n_tokens;) {
            ...
            if (verify_hooks) {
                ...
                chunk_hooks.require_fused_q1 = verify_hooks->require_fused_q1;
    ...
    if (fused_verify_candidate) {
        ...
        // A fused candidate deliberately bypassed the generic compressor
    ...
    // q=1 verification uses this exact ordinary-decode graph. Capture tensors
    // are passive additional outputs, so target logits and cache mutations are
    // identical to autoregressive decoding.
    if (n_tokens == 1 && allow_decode_graph_reuse &&
        layer_begin == 0 && is_last_shard &&
        ds4_backend_is_gpu(backend) && w.fused_decode) {
        const int rc = ds4_try_fused_decode_step(
            ...
            embed, kv_start, out_logits, token_ids, telemetry, verify_hooks);
        ...
        if (verify_hooks && verify_hooks->require_fused_q1) {
            std::fprintf(stderr,
                         "[deepseek4] exact DSpark q=1 graph unavailable; "
                         "refusing non-equivalent verifier fallback\n");
            return false;
        }
    } else if (n_tokens == 1 && verify_hooks &&
               verify_hooks->require_fused_q1) {
        std::fprintf(stderr,
                     "[deepseek4] exact DSpark q=1 graph is unsupported "
    ...
    auto capture_hc_layer = [&](int il, const float * state) {
        if (!verify_hooks || !verify_hooks->capture_layer_ids ||
            !verify_hooks->capture_out || !state) {
            return;
        }
        const std::vector<int> & ids = *verify_hooks->capture_layer_ids;
        for (size_t ci = 0; ci < ids.size(); ++ci) {
            if (ids[ci] != il) continue;
            const int ncap = (int) ids.size();
            std::vector<float> & captured = *verify_hooks->capture_out;
            if ((int) captured.size() != ncap * n_embd * n_tokens) {
                captured.assign(
                    (size_t) ncap * n_embd * n_tokens, 0.0f);
            }
            for (int t = 0; t < n_tokens; ++t) {
                float * dst =
                    captured.data() + (size_t) t * ncap * n_embd +
                    ci * n_embd;
                const float * hs =
                    state + (size_t) t * hc_dim;
                for (int d = 0; d < n_embd; ++d) {
                    float acc = 0.0f;
                    for (int h = 0; h < n_hc; ++h) {
                        acc += hs[(size_t) h * n_embd + d];
                    }
                    dst[d] = acc / (float) n_hc;
                }
            }
        }
    };
            ...
            if (use_backend_decode_hc_graph || use_backend_decode_hc_direct) {
                ...
                if (verify_hooks && verify_hooks->capture_layer_ids &&
                    ...
                    const std::vector<int> & ids =
                        *verify_hooks->capture_layer_ids;
                    if (std::find(ids.begin(), ids.end(), il) != ids.end()) {
                        ggml_backend_tensor_get(
...
// Compressed KV tensors are allocated for max_ctx in the live cache, but only
// rows [0, n_comp) are meaningful.  Snapshotting the full capacity made every
// resident slot and every disk checkpoint scale with max_ctx instead of the
// prefix it represented.  Keep at least one row so the tensor remains present
// in the serialized layout when the logical count is zero.
ggml_tensor * clone_snapshot_tensor_rows(ggml_context * ctx,
                                         const ggml_tensor * src,
                                         int rows,
                                         const char * name) {
    if (!ctx || !src || ggml_n_dims(src) != 2 || rows < 0 ||
        rows > src->ne[1]) {
        return nullptr;
    }
    const int64_t stored_rows = std::max<int64_t>(1, rows);
    ggml_tensor * dst =
        ggml_new_tensor_2d(ctx, src->type, src->ne[0], stored_rows);
    if (!dst) return nullptr;
    if (name && *name) ggml_set_name(dst, name);
    return dst;
}
...
bool copy_tensor_prefix_from_backend(const ggml_tensor * src,
    ...
    for (int i = 0; i < GGML_MAX_DIMS; ++i) {
        if (i == 1) {
            if (dst->ne[i] > src->ne[i]) return false;
        } else if (src->ne[i] != dst->ne[i]) {
            return false;
        }
    }
    const size_t bytes = ggml_nbytes(dst);
    if (bytes > ggml_nbytes(src)) return false;
    ggml_backend_tensor_get(src, dst->data, 0, bytes);
    return true;
}

bool copy_logical_tensor_prefix_from_backend(const ggml_tensor * src,
                                             ggml_tensor * dst,
                                             int logical_rows) {
    if (!src || !dst || logical_rows < 0) return false;
    if (logical_rows == 0) {
        std::memset(dst->data, 0, ggml_nbytes(dst));
        return true;
    }
    return copy_tensor_prefix_from_backend(src, dst);
}
...
bool tensor_prefix_compatible(const ggml_tensor * snap,
                              const ggml_tensor * live,
                              int logical_rows) {
    if (!!snap != !!live) return false;
    if (!snap) return logical_rows == 0;
    if (logical_rows < 0 || snap->type != live->type) {
        return false;
    }
    // A zero-logical-row prefix is represented by one physical placeholder
    // row so it remains serializable. GGML reports ne[1] == 1 tensors as
    // effectively one-dimensional even when they were created with
    // ggml_new_tensor_2d, while the live capacity tensor reports two. The
    // complete ne[] comparison below is the authoritative compatibility test;
    // requiring ggml_n_dims equality rejects this valid empty prefix.
    for (int i = 0; i < GGML_MAX_DIMS; ++i) {
        if (i == 1) {
            if (snap->ne[i] > live->ne[i] ||
                snap->ne[i] < std::max<int64_t>(1, logical_rows)) {
                return false;
            }
        } else if (snap->ne[i] != live->ne[i]) {
            return false;
        }
    }
    return true;
...
bool deepseek4_snapshot_save(const DeepSeek4Cache & cache,
                             ...
                             const std::vector<float> * host_spec_feat) {
    ...
    ggml_init_params ip{};
    // +8 globals covers hc_state, meta, logits, spec-feat and headroom.
    ...
    // Every tensor gets a deterministic name: snapshot_adopt() rebinds a
    // deserialized context by name, and unnamed tensors are unrecoverable.
    char nm[64];
    for (int il = 0; il < cache.n_layer; ++il) {
        const auto & src = cache.layers[(size_t)il];
        auto & dst = out.layers[(size_t)il];
        std::snprintf(nm, sizeof(nm), "ds4_raw_kv_%d", il);
        dst.raw_kv = clone_snapshot_tensor(out.ctx, src.raw_kv, nm);
        std::snprintf(nm, sizeof(nm), "ds4_comp_kv_%d", il);
        dst.comp_kv = src.comp_kv
            ? clone_snapshot_tensor_rows(out.ctx, src.comp_kv, src.n_comp, nm)
            : nullptr;
        std::snprintf(nm, sizeof(nm), "ds4_index_comp_kv_%d", il);
        dst.index_comp_kv = src.index_comp_kv
            ? clone_snapshot_tensor_rows(out.ctx, src.index_comp_kv,
                                         src.n_index_comp, nm)
            : nullptr;
        std::snprintf(nm, sizeof(nm), "ds4_attn_ckv_%d", il);
        dst.attn_compressor.state_kv =
            clone_snapshot_tensor(out.ctx, src.attn_compressor.state_kv, nm);
        std::snprintf(nm, sizeof(nm), "ds4_attn_cscore_%d", il);
        dst.attn_compressor.state_score =
            clone_snapshot_tensor(out.ctx, src.attn_compressor.state_score, nm);
        std::snprintf(nm, sizeof(nm), "ds4_idx_ckv_%d", il);
        dst.indexer_compressor.state_kv =
            clone_snapshot_tensor(out.ctx, src.indexer_compressor.state_kv, nm);
        std::snprintf(nm, sizeof(nm), "ds4_idx_cscore_%d", il);
        dst.indexer_compressor.state_score =
            clone_snapshot_tensor(out.ctx, src.indexer_compressor.state_score, nm);
        if (!dst.raw_kv ||
    ...
    // Non-tensor state, carried as tensors so the disk format persists it.
    out.meta_snap = ggml_new_tensor_1d(out.ctx, GGML_TYPE_I32,
                                       (int64_t)cache.n_layer * 2);
    if (!out.meta_snap) {
        free_deepseek4_snapshot(out);
        return false;
    }
    ggml_set_name(out.meta_snap, "ds4_snap_meta");

    if (host_logits && !host_logits->empty()) {
        out.logits_snap = ggml_new_tensor_1d(out.ctx, GGML_TYPE_F32,
                                             (int64_t)host_logits->size());
        if (!out.logits_snap) {
            free_deepseek4_snapshot(out);
            return false;
        }
        ggml_set_name(out.logits_snap, "ds4_snap_logits");
    }
    // The DSpark rolling window is deliberately NOT persisted. Its size varies
    // with how warm the drafter is, so emitting it conditionally made the set
    // of tensors — and therefore the disk layout fingerprint — depend on
    // runtime state. Two fingerprints partition the cache into halves that can
    // never see each other's files. A restored checkpoint simply rewarms the
    // window; only speculative accept-rate is affected, not correctness.
    (void)host_spec_feat;
    ...
    if (out.logits_snap) {
        ggml_backend_tensor_set(out.logits_snap, host_logits->data(), 0,
    ...
    for (int il = 0; il < cache.n_layer; ++il) {
        ...
        if (!copy_tensor_from_backend(src.raw_kv, dst.raw_kv) ||
            (src.comp_kv &&
             !copy_logical_tensor_prefix_from_backend(
    ...
    // Mirror the per-layer compressor counters into the meta tensor.
    {
        std::vector<int32_t> meta((size_t)cache.n_layer * 2);
        for (int il = 0; il < cache.n_layer; ++il) {
            meta[(size_t)il * 2 + 0] = out.layers[(size_t)il].n_comp;
            meta[(size_t)il * 2 + 1] = out.layers[(size_t)il].n_index_comp;
        }
        ggml_backend_tensor_set(out.meta_snap, meta.data(), 0,
...
bool deepseek4_snapshot_restore(const DeepSeek4Snapshot & snap,
    ...
    if (!snap.ctx || !cache.ctx || !cache.buf || !snap.hc_state_snap ||
        ...
        cache.layers.size() != (size_t) cache.n_layer ||
        snap.cur_pos < 0 ||
        (cache.max_ctx > 0 && snap.cur_pos > cache.max_ctx)) {
        std::fprintf(stderr,
                     "[deepseek4] snapshot restore rejected header: "
                     "snap_ctx=%d live_ctx=%d live_buf=%d hc=%d "
                     "snap_layers=%zu live_layers=%zu n_layer=%d "
                     "pos=%d max_ctx=%d\n",
                     snap.ctx != nullptr, cache.ctx != nullptr,
                     cache.buf != nullptr, snap.hc_state_snap != nullptr,
                     snap.layers.size(), cache.layers.size(), cache.n_layer,
                     snap.cur_pos, cache.max_ctx);
        return false;
    }
    if (!tensors_compatible(snap.hc_state_snap, cache.hc_state)) {
        std::fprintf(stderr,
                     "[deepseek4] snapshot restore rejected HC state shape\n");
        return false;
    }

    // Validate the complete snapshot before mutating the live cache. This
    // keeps restore transactional: an incompatible disk entry cannot leave a
    // partially restored mixture of old and new layers.
    for (size_t il = 0; il < cache.layers.size(); ++il) {
        ...
        const bool comp_ok = tensor_prefix_compatible(
            src.comp_kv, dst.comp_kv, src.n_comp);
        const bool index_ok = tensor_prefix_compatible(
        ...
        if (!raw_ok || !comp_ok || !index_ok || !attn_kv_ok ||
            ...
            std::fprintf(stderr,
                         "[deepseek4] snapshot restore rejected layer %zu "
                         "shape: raw=%d comp=%d index=%d attn_kv=%d "
                         "attn_score=%d index_kv=%d index_score=%d "
                         "n_comp=%d n_index=%d\n",
                         il, raw_ok, comp_ok, index_ok, attn_kv_ok,
                         attn_score_ok, index_kv_ok, index_score_ok,
                         src.n_comp, src.n_index_comp);
            return false;
        }
        const int comp_cap =
            dst.comp_kv ? (int) dst.comp_kv->ne[1] : 0;
        const int index_cap =
            dst.index_comp_kv ? (int) dst.index_comp_kv->ne[1] : 0;
        if (src.n_comp < 0 || src.n_comp > comp_cap ||
            src.n_index_comp < 0 || src.n_index_comp > index_cap) {
            std::fprintf(stderr,
                         "[deepseek4] snapshot counters out of range at "
                         "layer %zu: comp=%d/%d index=%d/%d\n",
                         il, src.n_comp, comp_cap,
                         src.n_index_comp, index_cap);
            return false;
        }
    }
    ...
    if (!copy_tensor_to_backend(snap.hc_state_snap, cache.hc_state)) {
        std::fprintf(stderr,
                     "[deepseek4] snapshot restore failed copying HC state\n");
        return false;
    }
    for (size_t il = 0; il < cache.layers.size(); ++il) {
        const auto & src = snap.layers[il];
        auto & dst = cache.layers[il];
        const bool raw_ok = copy_tensor_to_backend(src.raw_kv, dst.raw_kv);
        const bool comp_ok = !src.comp_kv || copy_tensor_prefix_to_backend(
            src.comp_kv, dst.comp_kv);
        const bool index_ok = !src.index_comp_kv ||
            copy_tensor_prefix_to_backend(
                src.index_comp_kv, dst.index_comp_kv);
        const bool attn_kv_ok = !src.attn_compressor.state_kv ||
            copy_tensor_to_backend(src.attn_compressor.state_kv,
                                   dst.attn_compressor.state_kv);
        const bool attn_score_ok = !src.attn_compressor.state_score ||
            copy_tensor_to_backend(src.attn_compressor.state_score,
                                   dst.attn_compressor.state_score);
        const bool index_kv_ok = !src.indexer_compressor.state_kv ||
            copy_tensor_to_backend(src.indexer_compressor.state_kv,
                                   dst.indexer_compressor.state_kv);
        const bool index_score_ok = !src.indexer_compressor.state_score ||
            copy_tensor_to_backend(src.indexer_compressor.state_score,
                                   dst.indexer_compressor.state_score);
        if (!raw_ok || !comp_ok || !index_ok || !attn_kv_ok ||
            !attn_score_ok || !index_kv_ok || !index_score_ok) {
            std::fprintf(stderr,
                         "[deepseek4] snapshot restore copy failed layer %zu: "
                         "raw=%d comp=%d index=%d attn_kv=%d "
                         "attn_score=%d index_kv=%d index_score=%d\n",
                         il, raw_ok, comp_ok, index_ok, attn_kv_ok,
                         attn_score_ok, index_kv_ok, index_score_ok);
            return false;
...
struct DsparkDraftCache {
    ...
    DsparkDraftCache() = default;
    DsparkDraftCache(const DsparkDraftCache &) = delete;
    DsparkDraftCache & operator=(const DsparkDraftCache &) = delete;
