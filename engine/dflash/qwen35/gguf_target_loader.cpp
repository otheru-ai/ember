...
bool CpuEmbedder::embed(const int32_t * ids, int n, float * out_f32) const {
    if (n < 0 || (n > 0 && (!ids || !out_f32)) ||
        !tok_embd_bytes || tok_embd_type == GGML_TYPE_COUNT ||
        n_embd <= 0 || n_vocab <= 0 || row_bytes == 0) return false;
...
static size_t align_up_size(size_t x, size_t a) {
    ...
    const size_t r = x % a;
    if (r == 0) return x;
    return x > SIZE_MAX - (a - r) ? SIZE_MAX : x + (a - r);
}
...
int read_target_layer_scales(const gguf_context * gctx,
    ...
    auto read_scale = [&](int il, const char * base) -> float {
        ...
        if (gguf_get_tensor_type(gctx, stid) != GGML_TYPE_F32 ||
            gguf_get_tensor_size(gctx, stid) != sizeof(float)) {
            return NAN;
        }
        const size_t srel = gguf_get_tensor_offset(gctx, stid);
        if (!gguf_tensor_in_file(data_start, srel, sizeof(float), file_len)) return NAN;
        float val;
    ...
    for (int il = 0; il < out.n_layer; il++) {
        ...
        auto count_s = [&](float s) {
            if (!std::isfinite(s)) n_scales = -1;
            else if (n_scales >= 0 && s != 1.0f) n_scales++;
        };
...
bool load_target_gguf_partial(const std::string & path,
    ...
    if (path.empty()) {
        set_last_error("empty target GGUF path");
        return false;
    }
    if (out.ctx || out.buf || !out.layers.empty() ||
        !out.embedder.tok_embd_owned.empty() || out.embedder.mmap_addr) {
        set_last_error("target output must be empty before load");
        return false;
    }
    ...
    std::unique_ptr<ggml_context, decltype(&ggml_free)> meta_guard(meta_ctx, &ggml_free);
    struct OutputRollback {
        ...
        ~OutputRollback() {
            ...
            out.out_norm = nullptr;
            out.output = nullptr;
            out.embedder.tok_embd_owned.clear();
            out.embedder.tok_embd_bytes = nullptr;
            out.embedder.tok_embd_type = GGML_TYPE_COUNT;
            out.embedder.row_bytes = 0;
        }
    } rollback{out};
    ...
    bool is_moe = false;
    {
        ...
        if (gguf_get_kv_type(gctx, arch_id) != GGUF_TYPE_STRING) {
    ...
    const auto fits_int = [](uint32_t value) {
        return value <= static_cast<uint32_t>(INT_MAX);
    };
    const bool invalid_common =
        n_embd == 0 || n_layer < 2 || n_head == 0 || n_headkv == 0 ||
        kl == 0 || vl == 0 || fai == 0 ||
        ssm_conv == 0 || ssm_inner == 0 || ssm_state == 0 ||
        ssm_dt == 0 || ssm_grp == 0 || ssm_inner % ssm_dt != 0 ||
        !fits_int(n_embd) || !fits_int(n_layer) || !fits_int(n_head) ||
        !fits_int(n_headkv) || !fits_int(kl) || !fits_int(vl) ||
        !fits_int(fai) || !fits_int(ssm_conv) || !fits_int(ssm_inner) ||
        !fits_int(ssm_state) || !fits_int(ssm_dt) || !fits_int(ssm_grp);
    const bool invalid_dense = !is_moe && (n_ff == 0 || !fits_int(n_ff));
    const bool invalid_moe = is_moe && (n_ff_exp == 0 || n_ff_shexp == 0 ||
                                        n_expert == 0 || n_expert_used == 0 ||
                                        n_expert_used > n_expert ||
                                        !fits_int(n_ff_exp) || !fits_int(n_ff_shexp) ||
                                        !fits_int(n_expert) || !fits_int(n_expert_used));
    ...
    int rope_sections[4] = {0, 0, 0, 0};
    {
        ...
        if (gguf_get_kv_type(gctx, rid) != GGUF_TYPE_ARRAY ||
            gguf_get_arr_type(gctx, rid) != GGUF_TYPE_INT32) {
            set_last_error("qwen35.rope.dimension_sections must be an int32 array");
            return false;
        }
    ...
    if (plan.expert_metadata_only) {
        if (!is_moe) {
            set_last_error("expert-metadata-only load requested for a dense model");
            return false;
        }
    ...
    if (!plan.metadata_only && !backend) {
        set_last_error("target load requires a backend unless metadata_only is set");
        return false;
    }

    const uint32_t rope_dimension_count =
        get_u32_or(gctx, key("rope.dimension_count").c_str(), 64);
    const float rope_theta =
        get_f32_or(gctx, key("rope.freq_base").c_str(), 10000000.0f);
    const float rms_eps =
        get_f32_or(gctx, key("attention.layer_norm_rms_epsilon").c_str(), 1e-6f);
    if (rope_dimension_count == 0 || !fits_int(rope_dimension_count) ||
        !std::isfinite(rope_theta) || rope_theta <= 0.0f ||
        !std::isfinite(rms_eps) || rms_eps <= 0.0f ||
        !std::isfinite(expert_weights_scale) || expert_weights_scale <= 0.0f) {
        set_last_error("invalid rope, normalization, or expert scale metadata");
        return false;
    }

    out.backend = plan.metadata_only ? nullptr : backend;
    ...
    out.rope_dimension_count = (int)rope_dimension_count;
    ...
    if (out.tok_embd->ne[0] <= 0 || out.tok_embd->ne[1] <= 0 ||
        out.tok_embd->ne[0] > INT_MAX || out.tok_embd->ne[1] > INT_MAX) {
        set_last_error("token_embd.weight dimensions are out of range");
        return false;
    }
    ...
    if (!plan.metadata_only) {
        ...
        if (!buft) {
            set_last_error("backend has no default buffer type");
            return false;
        }
        alignment = ggml_backend_buft_get_alignment(buft);
        if (alignment == 0) {
            set_last_error("backend reported zero buffer alignment");
            return false;
        }
    }
    ...
    if (n_tensors < 0 ||
        ...
        set_last_error("target GGUF tensor count is out of range");
        return false;
    }
    ...
    for (int64_t tid = 0; tid < n_tensors; tid++) {
        ...
        if (!t || (plan.expert_metadata_only
                       ? !(is_expert_tensor_name(tname) &&
                           [&]() {
                               int layer_id = -1;
                               return parse_block_tensor_name(tname, layer_id) &&
                                      layer_id >= plan.layer_begin &&
                                      layer_id < plan.layer_end;
                           }())
                       : !should_load_target_tensor(tname, plan.layer_begin,
        ...
        const size_t tensor_offset = gguf_get_tensor_offset(gctx, tid);
        if (tensor_offset > SIZE_MAX - data_offset) {
            set_last_error("target tensor file offset overflow");
            return false;
        }
        a.file_offset = data_offset + tensor_offset;
        a.file_size = gguf_get_tensor_size(gctx, tid);
        if (!plan.metadata_only) {
            alloc_total = align_up_size(alloc_total, alignment);
            if (alloc_total == SIZE_MAX) {
                set_last_error("target tensor alignment overflow");
                return false;
            }
            a.buffer_offset = alloc_total;
            const size_t alloc_size = ggml_backend_buft_get_alloc_size(buft, t);
            if (alloc_size > SIZE_MAX - alloc_total) {
                set_last_error("target GPU allocation size overflow");
                return false;
            }
    ...
    if (plan.metadata_only) {
        ...
        if (n_scales < 0) {
            set_last_error("invalid NVFP4 scale tensor");
            return false;
        }
        if (n_scales > 0) {
    ...
    for (int64_t tid = 0; tid < n_tensors; tid++) {
        ...
        if (sz > SIZE_MAX - total) {
            set_last_error("target upload byte count overflow");
            release_out_buffer();
            return false;
        }
        total += sz;
    ...
    const size_t expected_row = ggml_row_size(tok_embd_type, out.n_embd);
    if (tok_embd_sz == 0 || tok_embd_sz % static_cast<size_t>(out.n_vocab) != 0 ||
        tok_embd_sz / static_cast<size_t>(out.n_vocab) != expected_row ||
        out.tok_embd->ne[0] != out.n_embd ||
        out.tok_embd->ne[1] != out.n_vocab) {
        set_last_error("token_embd.weight has inconsistent shape or byte size");
        release_out_buffer();
        return false;
    }
