...
OUT_DIR=""
PREFILL_WORDS="${EMBER_PROFILE_PREFILL_WORDS:-2048}"
...
options:
  ...
  --prefill-words N    words in the prefill probe prompt (default 2048)
  --decode-tokens N    tokens generated by the decode probe (default 256)
  --gap-secs N         idle gap used to segment phases (default 3)
  --rocmi4-w4a8-iu4    enable the compiled exact gfx1151 W4A8 IU4 variant
  --resume             reuse response/CSV pairs from completed passes in --out-dir
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --rocmi4-w4a8-iu4) ROCMI4_W4A8_IU4=1; shift ;;
...
validate_response_shape() {
  ...
  [[ -s "$path" ]] || return 1
  python3 - "$path" "$phase" "$DECODE_TOKENS" <<'PY'
import json, sys
path, phase, expected_decode = sys.argv[1], sys.argv[2], int(sys.argv[3])
response = json.load(open(path, encoding="utf-8"))
usage = response.get("usage") or {}
if phase == "decode" and usage.get("completion_tokens") != expected_decode:
    raise SystemExit(
        f"profile decode shape differs: expected {expected_decode}, "
        f"got {usage.get('completion_tokens')!r}")
if phase == "prefill" and not isinstance(usage.get("prompt_tokens"), int):
    raise SystemExit("profile prefill response has no prompt-token count")
PY
}
...
pass_is_complete() {
  ...
  validate_response_shape "$outdir/response-$mode-$phase.json" "$phase" || return 1
  find "$outdir" -maxdepth 1 -type f -name "*$tag*kernel_trace.csv" -size +0c \
    -print -quit | grep -q . || return 1
  if [[ "$mode" == pmc ]]; then
    find "$outdir" -maxdepth 1 -type f -name "*$tag*counter_collection.csv" \
      -size +0c -print -quit | grep -q . || return 1
  fi
}
...
run_pass() {
  ...
  if (( RESUME )) && pass_is_complete "$mode" "$phase" "$outdir" "$counter"; then
    log "  [$tag] reusing completed response and profiler CSVs"
    return 0
  fi
  ...
  validate_response_shape "$outdir/response-$mode-$phase.json" "$phase"
