...
//
// Route-aware five-row ROCMFP4_FAST projection microkernel. Gate/up consume all
// rows because their route packet arrives after the activation stream. The
// down projection detects the exact zero tiles emitted for unowned rows and
// suppresses their MACs without retaining another FIFO object.
...
template <bool RouteMasked>
void gemm_impl(const bfloat16 * __restrict input,
    ...
    for (unsigned token = 0; token < kBatch; ++token) {
        if constexpr (RouteMasked) {
            const uint16_t * raw = reinterpret_cast<const uint16_t *>(
                input + token * kInput);
            uint16_t any = 0;
            for (unsigned lane = 0; lane < kInput; ++lane)
                any = static_cast<uint16_t>(any | raw[lane]);
            if (any == 0) continue;
        }
...
void zero_rocmfp4_v8_f32x5(float * output) {
...
void gemm_rocmfp4_v8_f32x5(const bfloat16 * __restrict input,
                           const uint8_t * __restrict weights,
                           float * __restrict output) {
    gemm_impl<false>(input, weights, output);
}

void gemm_rocmfp4_v8_masked_f32x5(
        const bfloat16 * __restrict input,
        const uint8_t * __restrict weights,
        float * __restrict output) {
    gemm_impl<true>(input, weights, output);
}
