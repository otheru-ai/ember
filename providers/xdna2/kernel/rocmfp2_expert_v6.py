...
"""Four-row weight-reusing ROCMFP2 shared-expert kernel for XDNA2."""
...
def build_expert():
    with mlir_mod_ctx() as ctx:
        ...
        def device_body():
            ...
            zero = external_func("zero_rocmfp2_v6_f32x4", inputs=[accum_ty],
                                 link_with="rocmfp2_gemm_v6.o")
            gemm = external_func("gemm_rocmfp2_v6_f32x4",
                                 inputs=[activation_ty, weight_ty, accum_ty],
                                 link_with="rocmfp2_gemm_v6.o")
            swiglu = external_func("swiglu_rocmfp2_v6_f32x4",
                                   inputs=[accum_ty, accum_ty, activation_ty, packet_ty],
                                   link_with="rocmfp2_expert_v6.o")
            store_down = external_func("store_down_rocmfp2_v6_f32x4",
            ...
            gate_acc = [[buffer(cores[row][col], accum_ty,
                                name=f"gate6_{row}_{col}") for col in range(COLS)]
                        for row in range(ROWS)]
            up_acc = [[buffer(cores[row][col], accum_ty,
                              name=f"up6_{row}_{col}") for col in range(COLS)]
                      for row in range(ROWS)]
            down_acc = [[[buffer(cores[row][col], accum_ty,
                                 name=f"down6_{group}_{row}_{col}")
            ...
            for col in range(COLS):
                ...
                mem_a[col] = object_fifo(f"mem_A6_{col}", shims[col], mems[col],
                                         2, activation_ty)
                in_a[col] = object_fifo(f"in_A6_{col}", mems[col], destinations,
                                        2, activation_ty)
                object_fifo_link(mem_a[col], in_a[col])
                in_hidden[col] = in_a[col]

                mem_w[col] = object_fifo(f"mem_W6_{col}", shims[col], mems[col],
                                         2, weight_mem_ty)
                for row in range(ROWS):
                    in_w[row][col] = object_fifo(
                        f"in_W6_{row}_{col}", mems[col], cores[row][col], 2,
                        weight_ty)
                ...
                mem_packet[col] = object_fifo(
                    f"mem_O6_{col}", mems[col], shims[col], 2, packet_mem_ty)
                for row in range(ROWS):
                    out_packet[row][col] = object_fifo(
                        f"out_O6_{row}_{col}", cores[row][col], mems[col], 2,
                        packet_ty)
