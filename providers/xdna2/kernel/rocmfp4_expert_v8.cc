...
//
// Route-aware DSpark epilogue. Route weights are applied before the linear
// down projection, so each expert run directly produces its weighted partial.
...
extern "C" void swiglu_rocmfp4_v8_f32x5(
    ...
    for (unsigned token = 0; token < kBatch; ++token) {
        const float route = raw_float(params + token * 128, 3);
        if (route == 0.0f) {
            const aie::vector<uint16, kLanes> zeros =
                aie::zeros<uint16, kLanes>();
            aie::store_v(raw_output + token * kPacketBf16, zeros);
            continue;
        }
        for (unsigned lane = 0; lane < kLanes; ++lane) {
            ...
            raw_output[token * kPacketBf16 + lane] = bf16_rne(
                (g / (1.0f + exp_approx(-g))) * u * route);
        }
    }
}

extern "C" void store_down_rocmfp4_v8_f32x5(
