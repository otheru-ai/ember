...
"""Five-row compensated Q8 shared expert for DSpark on XDNA2."""
...
EMBD = 4096
FF = 2048
Q8_TILE_BF16 = 2 * TILE_K * TILE_N
PACKED_BF16 = (EMBD * FF + EMBD * FF + FF * EMBD) * 2
    ...
    with mlir_mod_ctx() as ctx:
        ...
        def device_body():
            ...
            weight_mem_ty = np.ndarray[
                (ROWS * Q8_TILE_BF16,), np.dtype[bfloat16]
            ]
            ...
            zero = external_func("zero_q8_v2_f32x5", inputs=[accum_ty],
                                 link_with="q8_gemm_v2.o")
            gemm = external_func("gemm_q8_v2_f32x5",
            ...
            gate_acc = [[buffer(cores[row][col], accum_ty,
                                name=f"q8v2gate_{row}_{col}")
                         for col in range(COLS)] for row in range(ROWS)]
            up_acc = [[buffer(cores[row][col], accum_ty,
                              name=f"q8v2up_{row}_{col}")
                       for col in range(COLS)] for row in range(ROWS)]
            down_acc = [[[buffer(cores[row][col], accum_ty,
                                 name=f"q8v2down_{group}_{row}_{col}")
                          for col in range(COLS)] for row in range(ROWS)]
            ...
            for col in range(COLS):
                ...
                mem_a[col] = object_fifo(f"q8v2mem_A_{col}", shims[col], mems[col],
                                         2, activation_ty)
                in_a[col] = object_fifo(f"q8v2in_A_{col}", mems[col], destinations,
                                        2, activation_ty)
                object_fifo_link(mem_a[col], in_a[col])
                in_hidden[col] = in_a[col]
                mem_w[col] = object_fifo(f"q8v2mem_W_{col}", shims[col], mems[col],
                                         1, weight_mem_ty)
                for row in range(ROWS):
                    in_w[row][col] = object_fifo(
                        f"q8v2in_W_{row}_{col}", mems[col], cores[row][col], 1,
                        weight_ty)
                ...
                mem_packet[col] = object_fifo(
                    f"q8v2mem_O_{col}", mems[col], shims[col], 2, packet_mem_ty)
                for row in range(ROWS):
                    out_packet[row][col] = object_fifo(
                        f"q8v2out_O_{row}_{col}", cores[row][col], mems[col], 2,
                        packet_ty)
