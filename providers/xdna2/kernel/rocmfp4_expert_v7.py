...
"""Five-row weight-reusing ROCMFP4_FAST expert kernel for DSpark on XDNA2."""
...
EMBD = 4096
FF = 2048
V7_TILE_BYTES = TILE_K * TILE_N // 2 + (TILE_K // 32) * TILE_N * 2
...
def build_expert():
    with mlir_mod_ctx() as ctx:
        ...
        def device_body():
            ...
            weight_ty = np.ndarray[(V7_TILE_BYTES,), np.dtype[np.uint8]]
            weight_mem_ty = np.ndarray[(ROWS * V7_TILE_BYTES,), np.dtype[np.uint8]]
            ...
            zero = external_func("zero_rocmfp4_v7_f32x5", inputs=[accum_ty],
                                 link_with="rocmfp4_gemm_v7.o")
            gemm = external_func("gemm_rocmfp4_v7_f32x5",
                                 inputs=[activation_ty, weight_ty, accum_ty],
                                 link_with="rocmfp4_gemm_v7.o")
            swiglu = external_func("swiglu_rocmfp4_v7_f32x5",
                                   inputs=[accum_ty, accum_ty, activation_ty, packet_ty],
                                   link_with="rocmfp4_expert_v7.o")
            store_down = external_func("store_down_rocmfp4_v7_f32x5",
            ...
            gate_acc = [[buffer(cores[row][col], accum_ty,
                                name=f"gate7_{row}_{col}") for col in range(COLS)]
                        for row in range(ROWS)]
            up_acc = [[buffer(cores[row][col], accum_ty,
                              name=f"up7_{row}_{col}") for col in range(COLS)]
                      for row in range(ROWS)]
            down_acc = [[[buffer(cores[row][col], accum_ty,
                                 name=f"down7_{group}_{row}_{col}")
                          for col in range(COLS)] for row in range(ROWS)]
                        for group in range(2)]

            mem_a, mem_w, in_a, in_hidden = [None] * COLS, [None] * COLS, [None] * COLS, [None] * COLS
            ...
            for col in range(COLS):
                ...
                mem_a[col] = object_fifo(f"mem_A7_{col}", shims[col], mems[col],
                                         2, activation_ty)
                in_a[col] = object_fifo(f"in_A7_{col}", mems[col], destinations,
                                        2, activation_ty)
                object_fifo_link(mem_a[col], in_a[col])
                in_hidden[col] = in_a[col]

                mem_w[col] = object_fifo(f"mem_W7_{col}", shims[col], mems[col],
                                         2, weight_mem_ty)
                for row in range(ROWS):
                    in_w[row][col] = object_fifo(
                        f"in_W7_{row}_{col}", mems[col], cores[row][col], 2,
                        weight_ty)
                object_fifo_link(mem_w[col], [in_w[row][col] for row in range(ROWS)],
                                 [], [row * V7_TILE_BYTES for row in range(ROWS)])

                mem_packet[col] = object_fifo(
                    f"mem_O7_{col}", mems[col], shims[col], 2, packet_mem_ty)
                for row in range(ROWS):
                    out_packet[row][col] = object_fifo(
                        f"out_O7_{row}_{col}", cores[row][col], mems[col], 2,
                        packet_ty)
